  ...
  <div class="q-pa-md">
    <q-card>
      <q-card-section>
        <q-table
          class="table-title"
          title="Gestionar motivo de parálisis de medios de transportes"
        ...
        >
          <!--<template v-slot:top>
            <div class="text-subtitle1 text-weight-bold text-uppercase">
              Gestionar motivo de parálisis de medios de transportes
            </div>
            <q-space></q-space>
            <q-input
              dense
            ...
            <q-btn
              key="filter"
              dense
              :disable="mpmtransporte.length != 0 ? false : true"
              ...
              <q-card-section>
               <q-btn
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteMpmtransportes(props.row)"
                >
            ...
            <q-toolbar-title>
              {{
                this.accion == 'adicionar'
                  ? 'Adicionar motivo de parálisis de medios de transporte'
                  : 'Actualizar motivo de parálisis de medios de transporte'
              }}
            </q-toolbar-title>
        ...
        <q-card-section>
           <div class="row q-pa-sm">
              ...
              <q-input
                ...
                type="text"
                label="Motivo de parálisis de medios de transporte *"
                dense
                name="motivo"
                :error-message="mensajesError('motivo')"
                :error="$v.form_data.motivo.$error"
                debounce="1000"
                autogrow
              />
            </div>
...
export default {
  ...
  computed: {
    ...mapState('mpmtransporte', ['mpmtransporte']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('mpmtransporte', [
      'getMpmtransporte',
      'saveMpmtransporte',
      'editMpmtransporte',
      'deleteMpmtransporte',
    ...
    mensajesError(campo) {
      ...
      if (campo === 'motivo') {
        if (!this.$v.form_data.motivo.required) {
          this.required_error = true;
          return '';
        }
        if (!this.$v.form_data.motivo.alpha) {
          this.required_error = true;
          return 'Solo se aceptan caracteres alfabéticos';
        }
        if (!this.$v.form_data.motivo.maxLength) {
          this.required_error = false;
          return 'El motivo de parálisis de medios de transporte solo puede tener 30 carácteres';
        }
      }
    },
    filterFn(val, update) {
    ...
    async loadData(props) {
      ...
      await this.getMpmtransporte(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveMpmtransporte(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editMpmtransporte(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteMpmtransportes(selected) {
      if (selected) {
        this.setFormData(selected);
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteMpmtransporte(this.form_data.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
    ...
    motivo() {
      ...
      for (var i = 0; i < this.rolesNombre.length; i++) {
        if (this.form_data.motivo === this.rolesNombre[i].name) {
          mio = true;
        } else {
          mio1 = true;
        }
