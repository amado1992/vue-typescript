        ...
        >
          <template v-slot:top>
            <div class="text-subtitle1 text-weight-bold text-uppercase">
              Gestionar tipo de vehículo especializado
            </div>
            <q-space></q-space>
            <q-input
              dense
            ...
            <q-btn
              key="filter"
              dense
              :disable="tipo_vespecializado.length != 0 ? false : true"
            ...
            <q-td :props="props">
              <div class="q-gutter-xs">
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteTipo_vespecializados(props.row)"
                >
            ...
            <q-toolbar-title>
              {{
                this.accion == 'adicionar'
                  ? 'Adicionar tipo de vehículo especializado'
                  : 'Actualizar tipo de vehículo especializado'
              }}
            </q-toolbar-title>
        ...
        <q-card-section>
          <div class="row q-pa-sm">
              ...
              <q-input
                ...
                type="text"
                label="Tipo de vehículo especializado *"
                dense
                name="tipo"
...
export default {
  ...
  mounted() {
    this.addToHomeBreadcrumbs([
      ...
      { label: 'Nomencladores' },
      { label: 'Tipos de vehículos especializados' },
    ]);
  },
  watch: {
    pagination: {
  ...
  computed: {
    ...mapState('tipo_vespecializado', ['tipo_vespecializado']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('tipo_vespecializado', [
      'getTipo_vespecializado',
      'saveTipo_vespecializado',
      'editTipo_vespecializado',
      'deleteTipo_vespecializado',
    ]),
    ...
    mensajesError(campo) {
      ...
      if (campo === 'tipo') {
        ...
        if (!this.$v.form_data.tipo.maxLength) {
          this.required_error = false;
          return 'El tipo de vehículo especializado solo puede tener 30 carácteres';
        }
      }
    },
    filterFn(val, update) {
    ...
    async loadData(props) {
      ...
      await this.getTipo_vespecializado(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveTipo_vespecializado(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editTipo_vespecializado(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteTipo_vespecializados(selected) {
      if (selected) {
        this.setFormData(selected);
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteTipo_vespecializado(this.form_data.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
