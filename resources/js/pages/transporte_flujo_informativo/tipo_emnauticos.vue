        ...
        >
          <template v-slot:top>
            <div class="text-subtitle1 text-weight-bold text-uppercase">
              Gestionar tipo de embarcaciones y medios náuticos
            </div>
            <q-space></q-space>
            <q-input
              dense
            ...
            <q-btn
              key="filter"
              dense
              :disable="tipo_emnautico.length != 0 ? false : true"
            ...
            >
             ...
             <template v-slot:item="props">
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteTipo_emnauticos(props.row)"
                >
            ...
            <q-toolbar-title>
              {{
                this.accion == 'adicionar'
                  ? 'Adicionar tipo de embarcaciones y medios náuticos'
                  : 'Actualizar tipo de embarcaciones y medios náuticos'
              }}
            </q-toolbar-title>
        ...
        <q-card-section>
          <div class="row q-pa-sm">
              ...
              <q-input
                ...
                type="text"
                label="Tipo de embarcación y medio náutico *"
                dense
                name="tipo"
...
export default {
  ...
  mounted() {
    this.addToHomeBreadcrumbs([
      ...
      { label: 'Nomencladores' },
      { label: 'Tipos de embarcaciones y medios náuticos' },
    ]);
  },
  watch: {
    pagination: {
  ...
  computed: {
    ...mapState('tipo_emnautico', ['tipo_emnautico']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('tipo_emnautico', [
      'getTipo_emnautico',
      'saveTipo_emnautico',
      'editTipo_emnautico',
      'deleteTipo_emnautico',
    ]),
    ...
    mensajesError(campo) {
      ...
      if (campo === 'tipo') {
        ...
        if (!this.$v.form_data.tipo.maxLength) {
          this.required_error = false;
          return 'El tipo de embarcación y medio náutico solo puede tener 30 carácteres';
        }
      }
    },
    filterFn(val, update) {
    ...
    async loadData(props) {
      ...
      await this.getTipo_emnautico(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveTipo_emnautico(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editTipo_emnautico(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteTipo_emnauticos(selected) {
      if (selected) {
        this.setFormData(selected);
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteTipo_emnautico(this.form_data.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
