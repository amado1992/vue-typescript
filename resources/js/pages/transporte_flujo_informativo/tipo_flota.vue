        ...
        >
          <template v-slot:top>
            ...
            <q-btn
              key="filter"
              dense
              :disable="tipo_flota.length != 0 ? false : true"
            ...
            <q-td :props="props">
              <div class="q-gutter-xs">
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteTipo_Flotas(props.row)"
                >
            ...
            <q-toolbar-title>
              {{
                this.accion == 'adicionar'
                  ? 'Adicionar tipo de flota'
                  : 'Actualizar tipo de flota'
              }}
            </q-toolbar-title>
...
export default {
  ...
  data() {
    return {
      visibleColumns: ['tipo_flota','action'],
      modalDialog: false,
      permiso: true,
      val: false,
      selected: [],
  ...
  computed: {
    ...mapState('tipo_flota', ['tipo_flota']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('tipo_flota', [
      'getTipo_flota',
      'saveTipo_flota',
      'editTipo_flota',
      'deleteTipo_flota',
    ...
    mensajesError(campo) {
      ...
      if (campo === 'tipo_flota') {
        ...
        if (!this.$v.form_data.tipo_flota.alpha) {
          this.required_error = true;
          return 'Solo se aceptan caracteres alfabéticos';
        }
        if (!this.$v.form_data.tipo_flota.maxLength) {
          this.required_error = false;
          return 'El tipo de flota solo puede tener 30 carácteres';
        }
      }
    },
    filterFn(val, update) {
    ...
    async loadData(props) {
      ...
      await this.getTipo_flota(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveTipo_flota(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editTipo_flota(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteTipo_Flotas(selected) {
      if (selected) {
        this.setFormData(selected);
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteTipo_flota(this.form_data.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
