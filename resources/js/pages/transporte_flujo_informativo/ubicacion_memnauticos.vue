  ...
  <div class="q-pa-md">
    <q-card>
      <q-card-section>
        <q-table
          class="table-title"
          title="Gestionar ubicación del motor en embarcaciones y medios náuticos"
          :grid="xssmallScreen || smallScreen"
          table-header-class="text-uppercase"
          :data="ubicacion_memnautico"
          :columns="columns"
          row-key="id"
        ...
        >
          <!--<template v-slot:top>
            <div class="text-subtitle1 text-weight-bold text-uppercase">
              Gestionar ubicación del motor en embarcaciones y medios náuticos
            </div>
            <q-space></q-space>
            <q-input
              dense
            ...
            <q-btn
              key="filter"
              dense
              :disable="ubicacion_memnautico.length != 0 ? false : true"
            ...
            <q-card class="q-ma-xs">
              <q-card-section>
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteUbicacion_memnauticos(props.row)"
                >
      ...
      <q-card style="width: 600px; max-width: 70vw;">
            ...
            <q-toolbar-title>
              {{
                this.accion == 'adicionar'
                  ? 'Adicionar ubicación del motor'
                  : 'Actualizar ubicación del motor'
              }}
            </q-toolbar-title>
        ...
        <q-card-section>
           <div class="row q-pa-sm">
              ...
              <q-input
                v-model="form_data.ubicacion"
                type="text"
                label="Ubicación del motor en embarcación y medio náutico *"
                dense
                name="ubicacion"
                :error-message="mensajesError('ubicacion')"
                :error="$v.form_data.ubicacion.$error"
                debounce="1000"
                autogrow
              />
            </div>
...
export default {
  name: 'ubicacion_memnauticos',
  mixins: [SmallScreen],
  data() {
    return {
      visibleColumns: ['ubicacion','action'],
      modalDialog: false,
      permiso: true,
      val: false,
      selected: [],
  ...
  mounted() {
    this.addToHomeBreadcrumbs([
      ...
      { label: 'Nomencladores' },
      { label: 'Ubicación del motor en embarcaciones y medios náuticos' },
    ]);
  },
  watch: {
    pagination: {
  ...
  computed: {
    ...mapState('ubicacion_memnautico', ['ubicacion_memnautico']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('ubicacion_memnautico', [
      'getUbicacion_memnautico',
      'saveUbicacion_memnautico',
      'editUbicacion_memnautico',
      'deleteUbicacion_memnautico',
    ]),
    ...
    mensajesError(campo) {
      ...
      if (campo === 'ubicacion') {
        if (!this.$v.form_data.ubicacion.required) {
          this.required_error = true;
          return '';
        }
        if (!this.$v.form_data.ubicacion.alpha) {
          this.required_error = true;
          return 'Solo se aceptan caracteres alfabéticos';
        }
        if (!this.$v.form_data.ubicacion.maxLength) {
          this.required_error = false;
          return 'La ubicación del motor en la embarcación y medio náutico solo puede tener 30 carácteres';
        }
      }
    },
    filterFn(val, update) {
    ...
    async loadData(props) {
      ...
      await this.getUbicacion_memnautico(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveUbicacion_memnautico(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editUbicacion_memnautico(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteUbicacion_memnauticos(selected) {
      if (selected) {
        this.setFormData(selected);
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteUbicacion_memnautico(this.form_data.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
