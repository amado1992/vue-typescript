        ...
        >
          <template v-slot:top>
            <div class="text-subtitle1 text-weight-bold text-uppercase">
              Gestionar tipo de vehículos administrativos y de servicios
            </div>
            <q-space></q-space>
            <q-input
              dense
            ...
            <q-btn
              key="filter"
              dense
              :disable="tipo_vaservicio.length != 0 ? false : true"
            ...
            <q-td :props="props">
              <div class="q-gutter-xs">
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteTipo_vaservicios(props.row)"
                >
            ...
            <q-toolbar-title>
              {{
                this.accion == 'adicionar'
                  ? 'Adicionar tipo de vehículo administrativo y de servicio'
                  : 'Actualizar tipo de vehículo administrativo y de servicio'
              }}
            </q-toolbar-title>
        ...
        <q-card-section>
           <div class="row q-pa-sm">
              ...
              <q-input
                ...
                type="text"
                label="Tipo de vehículo administrativo y de servicio *"
                dense
                name="tipo"
                :error-message="mensajesError('tipo')"
                :error="$v.form_data.tipo.$error"
                debounce="1000"
                autogrow
              />
            </div>
...
export default {
  name: 'tipo_vaservicios',
  mixins:[SmallScreen],
  data() {
    return {
      visibleColumns: ['tipo','action'],
      modalDialog: false,
      permiso: true,
      val: false,
      selected: [],
      ...
      columns: [
        ...
        {
          name: 'tipo',
          label: 'Tipo de vehículos administrativos y de servicios',
          align: 'left',
          field: 'tipo',
          search: true,
        },
  ...
  mounted() {
    this.addToHomeBreadcrumbs([
      ...
      { label: 'Nomencladores' },
      { label: 'Tipos de vehículos administrativos y de servicios' },
    ]);
  },
  watch: {
    pagination: {
  ...
  computed: {
    ...mapState('tipo_vaservicio', ['tipo_vaservicio']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('tipo_vaservicio', [
      'getTipo_vaservicio',
      'saveTipo_vaservicio',
      'editTipo_vaservicio',
      'deleteTipo_vaservicio',
    ]),
    ...
    mensajesError(campo) {
      ...
      if (campo === 'tipo') {
        ...
        if (!this.$v.form_data.tipo.alpha) {
          this.required_error = true;
          return 'Solo se aceptan caracteres alfabéticos';
        }
        if (!this.$v.form_data.tipo.maxLength) {
          this.required_error = false;
          return 'El tipo de vehículo administrativo y de servicio solo puede tener 30 carácteres';
        }
      }
    },
    filterFn(val, update) {
    ...
    async loadData(props) {
      ...
      await this.getTipo_vaservicio(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveTipo_vaservicio(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editTipo_vaservicio(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteTipo_vaservicios(selected) {
      if (selected) {
        this.setFormData(selected);
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteTipo_vaservicio(this.form_data.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
    ...
    tipo() {
      ...
      for (var i = 0; i < this.rolesNombre.length; i++) {
        if (this.form_data.tipo === this.rolesNombre[i].name) {
          mio = true;
        } else {
          mio1 = true;
        }
