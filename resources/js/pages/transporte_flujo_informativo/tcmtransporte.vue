  ...
  <div class="q-pa-md">
    <q-card>
      <q-card-section>
        <q-table
        class="table-title"
          title="Gestionar tipo de combustible de medios de transportes"
        ...
        >
          <!--<template v-slot:top>
            <div class="text-subtitle1 text-weight-bold text-uppercase">
              Gestionar tipo de combustible de medios de transportes
            </div>
            <q-space></q-space>
            <q-input
              dense
            ...
            <q-btn
              key="filter"
              dense
              :disable="tcmtransporte.length != 0 ? false : true"
            ...
            <q-card class="q-ma-xs">
              <q-card-section>
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteTcmtransportes(props.row)"
                >
      ...
      <q-card style="width: 600px; max-width: 70vw;">
            ...
            <q-toolbar-title>
              {{
                this.accion == 'adicionar'
                  ? 'Adicionar tipo de combustile de medios de transporte'
                  : 'Actualizar tipo de combustible de medios de transporte'
              }}
            </q-toolbar-title>
        ...
        <q-card-section>
           <div class="row q-pa-sm">
              ...
              <q-input
                v-model="form_data.combustible"
                type="text"
                label="Tipo de combustible de medios de transporte *"
                dense
                name="combustible"
                :error-message="mensajesError('combustible')"
                :error="$v.form_data.combustible.$error"
                debounce="1000"
                autogrow
              />
            </div>
...
export default {
  ...
  data() {
    return {
      visibleColumns: ['combustible','action'],
      modalDialog: false,
      permiso: true,
      val: false,
      selected: [],
  ...
  computed: {
    ...mapState('tcmtransporte', ['tcmtransporte']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('tcmtransporte', [
      'getTcmtransporte',
      'saveTcmtransporte',
      'editTcmtransporte',
      'deleteTcmtransporte',
    ...
    mensajesError(campo) {
      ...
      if (campo === 'combustible') {
        if (!this.$v.form_data.combustible.required) {
          this.required_error = true;
          return '';
        }
        if (!this.$v.form_data.combustible.alpha) {
          this.required_error = true;
          return 'Solo se aceptan caracteres alfabéticos';
        }
        if (!this.$v.form_data.combustible.maxLength) {
          this.required_error = false;
          return 'El tipo de combustible de medios de transporte solo puede tener 30 carácteres';
        }
      }
    },
    filterFn(val, update) {
    ...
    async loadData(props) {
      ...
      await this.getTcmtransporte(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveTcmtransporte(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editTcmtransporte(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteTcmtransportes(selected) {
      if (selected) {
        this.setFormData(selected);
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteTcmtransporte(this.form_data.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
    ...
    combustible() {
      ...
      for (var i = 0; i < this.rolesNombre.length; i++) {
        if (this.form_data.combustible === this.rolesNombre[i].name) {
          mio = true;
        } else {
          mio1 = true;
        }
