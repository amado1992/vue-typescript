  ...
  <div class="q-pa-md">
    <q-card>
      <q-card-section>
        <q-table
          class="table-title"
          title="Gestionar vehiculos ajenos"
        ...
        >
          <!--<template v-slot:top>
            ...
            <q-btn
              key="filter"
              dense
              :disable="vehiculo_ajeno.length != 0 ? false : true"
            ...
            <q-card class="q-ma-xs">
              <q-card-section>
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteVehiculo_ajenos(props.row)"
                >
            ...
            <q-toolbar-title>
              {{
                this.accion == 'adicionar'
                  ? 'Adicionar vehiculo ajeno'
                  : 'Actualizar vehiculo ajeno'
              }}
            </q-toolbar-title>
...
export default {
  name: 'vehiculo_ajenos',
  mixins:[SmallScreen],
  data() {
    return {
      visibleColumns: ['tipo', 'marca', 'matricula', 'action'],
      modalDialog: false,
      permiso: true,
      val: false,
      selected: [],
  ...
  computed: {
    ...mapState('vehiculo_ajeno', ['vehiculo_ajeno']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('vehiculo_ajeno', [
      'getVehiculo_ajeno',
      'saveVehiculo_ajeno',
      'editVehiculo_ajeno',
      'deleteVehiculo_ajeno',
    ]),
    ...
    async loadData(props) {
      ...
      await this.getVehiculo_ajeno(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveVehiculo_ajeno(this.form_data);
        this.reset_form();
        this.closeModal();
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editVehiculo_ajeno(this.form_data);
        this.reset_form();
        this.closeModal();
        this.loadData({ pagination: this.pagination, filter: this.filter });
        this.setLoading(false);
    ...
    deleteVehiculo_ajenos(selected) {
      if (selected) {
        this.setFormData(selected);
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteVehiculo_ajeno(this.form_data.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
