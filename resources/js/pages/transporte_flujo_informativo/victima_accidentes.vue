        ...
        >
          <!--<template v-slot:top>
            ...
            <q-btn
              key="filter"
              dense
              :disable="victima_accidente.length != 0 ? false : true"
            ...
            <q-card class="q-ma-xs">
              <q-card-section>
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteVictima_accidentes(props.row)"
                >
      ...
      <q-card style="width: 600px; max-width: 70vw;">
            ...
            <q-toolbar-title>
              {{
                this.accion == 'adicionar'
                  ? 'Adicionar accidente con'
                  : 'Actualizar accidente con'
              }}
            </q-toolbar-title>
        ...
        <q-card-section>
           <div class="row q-pa-sm">
              ...
              <q-input
                v-model="form_data.victima_accidente"
                type="text"
                label="Accidente con *"
                dense
                name="victima_accidente"
                :error-message="mensajesError('victima_accidente')"
                :error="$v.form_data.victima_accidente.$error"
                debounce="1000"
                autogrow
              />
            </div>
...
export default {
  name: 'victima_accidentes',
  mixins: [SmallScreen],
  data() {
    return {
      visibleColumns: ['victima_accidente','action'],
      modalDialog: false,
      permiso: true,
      val: false,
      selected: [],
  ...
  computed: {
    ...mapState('victima_accidente', ['victima_accidente']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('victima_accidente', [
      'getVictima_accidente',
      'saveVictima_accidente',
      'editVictima_accidente',
      'deleteVictima_accidente',
    ]),
    ...
    mensajesError(campo) {
      ...
      if (campo === 'victima_accidente') {
        if (!this.$v.form_data.victima_accidente.required) {
          this.required_error = true;
          return '';
        }
        if (!this.$v.form_data.victima_accidente.alpha) {
          this.required_error = true;
          return 'Solo se aceptan caracteres alfabéticos';
        }
        if (!this.$v.form_data.victima_accidente.maxLength) {
          this.required_error = false;
          return 'La victima del accidente solo puede tener 30 carácteres';
        }
      }
    },
    filterFn(val, update) {
    ...
    reset_form() {
      this.form_data.victima_accidente = '';
      this.form_error = false;
      this.val = false;
      this.required_error = false;
      this.permiso = true;
    ...
    async loadData(props) {
      ...
      await this.getVictima_accidente(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveVictima_accidente(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editVictima_accidente(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteVictima_accidentes(selected) {
      if (selected) {
        this.setFormData(selected);
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteVictima_accidente(this.form_data.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
