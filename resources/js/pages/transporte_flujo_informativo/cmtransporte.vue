  ...
  <div class="q-pa-md">
    <q-card>
      <q-card-section>
        <q-table
          class="table-title"
          title="Gestionar clase de medios de transportes"
        ...
        >
          <!--<template v-slot:top>
            <div class="text-subtitle1 text-weight-bold text-uppercase">
              Gestionar clase de medios de transportes
            </div>
            <q-space></q-space>
            <q-input
              dense
            ...
            <q-btn
              key="filter"
              dense
              :disable="cmtransporte.length != 0 ? false : true"
            ...
            <q-card class="q-ma-xs">
              <q-card-section>
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteCmtransportes(props.row)"
                >
            ...
            <q-toolbar-title>
              {{
                this.accion == 'adicionar'
                  ? 'Adicionar clase de medios de transporte'
                  : 'Actualizar clase de medios de transporte'
              }}
            </q-toolbar-title>
        ...
        <q-card-section>
           <div class="row q-pa-sm">
              ...
              <q-input
                ...
                type="text"
                label="Clase de medios de transporte *"
                dense
                name="clase"
...
export default {
  ...
  computed: {
    ...mapState('cmtransporte', ['cmtransporte']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('cmtransporte', [
      'getCmtransporte',
      'saveCmtransporte',
      'editCmtransporte',
      'deleteCmtransporte',
    ...
    mensajesError(campo) {
      ...
      if (campo === 'clase') {
        ...
        if (!this.$v.form_data.clase.alpha) {
          this.required_error = true;
          return 'Solo se aceptan caracteres alfabéticos';
        }
        if (!this.$v.form_data.clase.maxLength) {
          this.required_error = false;
          return 'La clase de medios de transporte solo puede tener 30 carácteres';
        }
      }
    },
    filterFn(val, update) {
    ...
    async loadData(props) {
      ...
      await this.getCmtransporte(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveCmtransporte(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editCmtransporte(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteCmtransportes(selected) {
      if (selected) {
        this.setFormData(selected);
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteCmtransporte(this.form_data.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
    ...
    clase() {
      ...
      for (var i = 0; i < this.rolesNombre.length; i++) {
        if (this.form_data.clase === this.rolesNombre[i].name) {
          mio = true;
        } else {
          mio1 = true;
        }
