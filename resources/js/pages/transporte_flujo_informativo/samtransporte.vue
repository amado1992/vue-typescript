  ...
  <div class="q-pa-md">
    <q-card>
      <q-card-section>
        <q-table
          class="table-title"
          title="Gestionar situación actual de medios de transportes"
        ...
        >
          <!--<template v-slot:top>
            <div class="text-subtitle1 text-weight-bold text-uppercase">
              Gestionar situación actual de medios de transportes
            </div>
            <q-space></q-space>
            <q-input
              dense
            ...
            <q-btn
              key="filter"
              dense
              :disable="samtransporte.length != 0 ? false : true"
              ...
              <q-card-section>
               <q-btn
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteSamtransportes(props.row)"
                >
            ...
            <q-toolbar-title>
              {{
                this.accion == 'adicionar'
                  ? 'Adicionar situación actual de medios de transporte'
                  : 'Actualizar situación actual de medios de transporte'
              }}
            </q-toolbar-title>
        ...
        <q-card-section>
           <div class="row q-pa-sm">
              ...
              <q-input
                ...
                type="text"
                label="Situación actual de medios de transporte *"
                dense
...
export default {
  ...
  data() {
    return {
      visibleColumns: ['situacion_actual','action'],
      modalDialog: false,
      permiso: true,
      val: false,
      selected: [],
  ...
  mounted() {
    this.addToHomeBreadcrumbs([
      ...
      { label: 'Nomencladores' },
      { label: 'Situacion actual de medios de transporte' },
    ]);
  },
  watch: {
    pagination: {
  ...
  computed: {
    ...mapState('samtransporte', ['samtransporte']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('samtransporte', [
      'getSamtransporte',
      'saveSamtransporte',
      'editSamtransporte',
      'deleteSamtransporte',
    ...
    mensajesError(campo) {
      ...
      if (campo === 'situacion_actual') {
        ...
        if (!this.$v.form_data.situacion_actual.alpha) {
          this.required_error = true;
          return 'Solo se aceptan caracteres alfabéticos';
        }
        if (!this.$v.form_data.situacion_actual.maxLength) {
          this.required_error = false;
          return 'La situación actual de medios de transporte solo puede tener 30 carácteres';
        }
      }
    },
    filterFn(val, update) {
    ...
    async loadData(props) {
      ...
      await this.getSamtransporte(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveSamtransporte(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editSamtransporte(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteSamtransportes(selected) {
      if (selected) {
        this.setFormData(selected);
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteSamtransporte(this.form_data.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
    ...
    situacion_actual() {
      ...
      for (var i = 0; i < this.rolesNombre.length; i++) {
        if (this.form_data.situacion_actual === this.rolesNombre[i].name) {
          mio = true;
        } else {
          mio1 = true;
        }
