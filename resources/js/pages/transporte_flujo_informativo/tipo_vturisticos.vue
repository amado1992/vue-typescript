        ...
        >
          <template v-slot:top>
            <div class="text-subtitle1 text-weight-bold text-uppercase">
              Gestionar tipo de vehículo turístico
            </div>
            <q-space></q-space>
            <q-input
              dense
            ...
            <q-btn
              key="filter"
              dense
              :disable="tipo_vturistico.length != 0 ? false : true"
            ...
            <q-td :props="props">
              <div class="q-gutter-xs">
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteTipo_vturisticos(props.row)"
                >
            ...
            <q-toolbar-title>
              {{
                this.accion == 'adicionar'
                  ? 'Adicionar tipo de vehículo turístico'
                  : 'Actualizar tipo de vehículo turístico'
              }}
            </q-toolbar-title>
        ...
        <q-card-section>
          <div class="row q-pa-sm">
              ...
              <q-input
                ...
                type="text"
                label="Tipo de vehículo turístico *"
                dense
                name="tipo"
...
export default {
  ...
  mounted() {
    this.addToHomeBreadcrumbs([
      ...
      { label: 'Nomencladores' },
      { label: 'Tipos de vehículos turísticos' },
    ]);
  },
  watch: {
    pagination: {
  ...
  computed: {
    ...mapState('tipo_vturistico', ['tipo_vturistico']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('tipo_vturistico', [
      'getTipo_vturistico',
      'saveTipo_vturistico',
      'editTipo_vturistico',
      'deleteTipo_vturistico',
    ]),
    ...
    mensajesError(campo) {
      ...
      if (campo === 'tipo') {
        ...
        if (!this.$v.form_data.tipo.maxLength) {
          this.required_error = false;
          return 'El tipo de vehículo turístico solo puede tener 30 carácteres';
        }
      }
    },
    filterFn(val, update) {
    ...
    async loadData(props) {
      ...
      await this.getTipo_vturistico(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveTipo_vturistico(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editTipo_vturistico(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteTipo_vturisticos(selected) {
      if (selected) {
        this.setFormData(selected);
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteTipo_vturistico(this.form_data.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
