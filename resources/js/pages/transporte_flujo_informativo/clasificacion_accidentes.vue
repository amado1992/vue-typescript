  ...
  <div class="q-pa-md">
    <q-card>
      <q-card-section>
        <q-table
          class="table-title"
          title="Gestionar clasificación de accidentes"
          :grid="xssmallScreen || smallScreen"
          table-header-class="text-uppercase"
          :data="clasificacion_accidente"
          :columns="columns"
          row-key="id"
        ...
        >
            ...
            >
              <template v-slot:append>
                <q-btn
                  key="filter"
                  dense
                  :disable="clasificacion_accidente.length != 0 ? false : true"
          ...
          <!-- <template v-slot:top>
            <div class="text-subtitle1 text-weight-bold text-uppercase">
              Gestionar clasificación de accidentes
            </div>
            <q-space></q-space>
            <q-input
              dense
            ...
            <q-td :props="props">
              <div class="q-gutter-xs">
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteClasificacion_accidentes(props.row)"
                >
            ...
            <q-toolbar-title>
              {{
                this.accion == "adicionar"
                  ? "Adicionar clasificación de accidente"
                  : "Actualizar clasificación de accidente"
              }}
            </q-toolbar-title>
        ...
        <q-card-section>
          <div class="row q-pa-sm">
            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
              <q-input
                v-model="form_data.clasificacion_accidente"
                type="text"
                label="Clasificación de accidente *"
                dense
                name="clasificacion_accidente"
                :error-message="mensajesError('clasificacion_accidente')"
                :error="$v.form_data.clasificacion_accidente.$error"
                debounce="1000"
                autogrow
              />
            </div>
...
export default {
  name: "clasificacion_accidentes",
  mixins: [SmallScreen],
  data() {
    return {
      visibleColumns: ["clasificacion_accidente", "action"],
      modalDialog: false,
      permiso: true,
      val: false,
      selected: [],
  ...
  mounted() {
    this.addToHomeBreadcrumbs([
      ...
      { label: "Nomencladores" },
      { label: "Clasificaciones de accidentes" },
    ]);
  },
  watch: {
    pagination: {
  ...
  computed: {
    ...mapState("clasificacion_accidente", ["clasificacion_accidente"]),
  },
  methods: {
    ...mapGetters("utils", ["getterLoading"]),
    ...mapActions("clasificacion_accidente", [
      "getClasificacion_accidente",
      "saveClasificacion_accidente",
      "editClasificacion_accidente",
      "deleteClasificacion_accidente",
    ]),
    ...
    mensajesError(campo) {
      ...
      if (campo === "clasificacion_accidente") {
        if (!this.$v.form_data.clasificacion_accidente.required) {
          this.required_error = true;
          return "";
        }
        if (!this.$v.form_data.clasificacion_accidente.alpha) {
          this.required_error = true;
          return 'Solo se aceptan caracteres alfabéticos';
        }
        if (!this.$v.form_data.clasificacion_accidente.maxLength) {
          this.required_error = false;
          return "La clasificación del accidente solo puede tener 30 carácteres";
        }
      }
    },
    filterFn(val, update) {
    ...
    reset_form() {
      this.form_data.clasificacion_accidente = "";
      this.form_error = false;
      this.val = false;
      this.required_error = false;
      this.permiso = true;
    ...
    async loadData(props) {
      ...
      await this.getClasificacion_accidente(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveClasificacion_accidente(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editClasificacion_accidente(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteClasificacion_accidentes(selected) {
      if (selected) {
        this.setFormData(selected);
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteClasificacion_accidente(this.form_data.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
