        ...
        >
          <template v-slot:top>
            ...
            <q-btn
              key="filter"
              dense
              :disable="tipo_proveedor.length != 0 ? false : true"
...
export default {
  ...
  computed: {
    ...mapState('tipo_proveedor', ['tipo_proveedor']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('usuario', ['getTotalUsuariosRol']),
    ...mapActions('tipo_proveedor', [
      'getTipo_proveedor',
      'saveTipo_proveedor',
      'editTipo_proveedor',
      'deleteTipo_proveedor',
    ]),
    ...
    async loadData(props) {
      ...
      await this.getTipo_proveedor(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveTipo_proveedor(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editTipo_proveedor(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteTipo_proveedores(selected) {
      if (selected) {
        this.setFormData(selected);
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteTipo_proveedor(this.form_data.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
