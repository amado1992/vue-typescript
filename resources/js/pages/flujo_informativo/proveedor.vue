        ...
        >
          <template v-slot:top>
            ...
            <q-btn
              key="filter"
              dense
              :disable="proveedor.length != 0 ? false : true"
            ...
            <q-td :props="props">
              <div class="q-gutter-xs">
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteProveedores(props.row)"
                >
      ...
      <q-card style="width: 600px; max-width: 70vw;">
        ...
        <q-card-section class="q-px-md">
          ...
          <div class="row">
            <div class="col-6">
              <q-select
                v-model="form_data.tipo_proveedor_id"
                emit-value
                map-options
                options-dense
                :options="items_tipo_proveedor"
                label="Tipo de proveedor *"
                option-label="nombre"
                option-value="id"
                :error-message="mensajesError('tipo_proveedor_id')"
                :error="$v.form_data.tipo_proveedor_id.$error"
              />
            </div>
            <div class="col-6 q-pl-sm">
              <q-select
                v-model="form_data.municipio_id"
                emit-value
                map-options
                options-dense
                :options="items_municipio"
                label="Municipio *"
                option-label="nombre"
                option-value="id"
                :error-message="mensajesError('municipio_id')"
                :error="$v.form_data.municipio_id.$error"
              />
            </div>
          </div>
...
export default {
  ...
  data() {
    return {
      visibleColumns: ['codigo', 'nombre', 'tipo_proveedor_id', 'municipio_id', 'action'],
      modalDialog: false,
      filterInput: false,
      permiso: true,
      val: false,
      ...
      columns: [
        ...
        },
        {
          ...
          align: 'left',
          label: 'Municipio',
          field: row => row.municipios !== undefined ? row.municipios.nombre : ''
        },
        {
          ...
          label: 'Tipo de proveedor',
          field: (row) =>
            row.proveedores !== undefined ? row.proveedores.nombre : '',
        },
        {
          name: 'action',
          align: 'center',
  ...
  mounted() {
    ...
    this.getTipo_proveedores().then((r) => {
      this.items_tipo_proveedor = r;
    });
  ...
  computed: {
    ...mapState('proveedor', ['proveedor']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('usuario', ['getTotalUsuariosRol']),
    ...mapActions('tipo_proveedor', ['getTipo_proveedores']),
    ...
    mensajesError(campo) {
      ...
      if (campo === 'municipio_id') {
        if (!this.$v.form_data.municipio_id.required) {
          this.required_error = true
          return ''
        }
      }
      if (campo === 'tipo_proveedor_id') {
        if (!this.$v.form_data.tipo_proveedor_id.required) {
          this.required_error = true;
          return '';
        }
      }
    ...
    reset_form() {
      ...
      this.form_data.tipo_proveedor_id = '';
      this.form_error = false;
      this.val = false;
      this.required_error = false;
      this.permiso = true;
    ...
    async loadData(props) {
      ...
      await this.getProveedor(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveProveedor(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editProveedor(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteProveedores(selected) {
      if (selected) {
        this.setFormData(selected);
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteProveedor(this.form_data.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
