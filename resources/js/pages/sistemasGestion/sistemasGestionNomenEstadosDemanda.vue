          ...
          <q-card-section class="no-padding">
            <q-card-section class="row no-padding">
              <q-toolbar class="bg-primary text-white">
                <q-toolbar-title>
                  {{
                    this.action == "add" ? "Crear Estado" : this.action == "edit" ? "Editar Estado" : "Eliminar Estado"
                  }}
                </q-toolbar-title>
                <q-btn
                  round
            ...
            <q-card-section>
              <div class="row">
                <div class="col" style="padding: 0px 20px 20px 0px">
                  <q-input v-if="action != 'delete'" name="desc" v-model="form_data.desc_EstadoDemanda" type="text" label="Descripción" dense/>
                  <label v-if="action == 'delete'"  name="desc1"  type="text"  dense
                  >
                    Está seguro de eliminar el estado: {{this.form_data.desc_EstadoDemanda}}</label>
                </div>
              </div>
              <br/>
...
import {getEstadosDemanda} from "../../store/sistema_gestion/actions";
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          label: 'Descripción',
          align: 'left',
          field: row => row.desc_EstadoDemanda,
          sortable: true,
      ...
      visibleColumns: ['columnname','columnactions'],
      form_data: {
        id:'',
        desc_EstadoDemanda:'',
      },
  ...
  mounted() {
    this.addToHomeBreadcrumbs([
      ...
      {label: 'Nomencladores'},
      {label: 'Gestionar Estados de Demanda'}])
    this.loadData()
  },
  watch: {
    pagination: {
  ...
  methods: {
    ...
    ...mapActions('managementsystem', ['getEstadosDemanda']),
    ...mapActions('managementsystem', ['addEstadoDemanda']),
    ...mapActions('managementsystem', ['editEstadoDemanda']),
    ...mapActions('managementsystem', ['deleteEstadoDemanda']),
    async loadData() {
      this.loading = true
      await this.getEstadosDemanda().then(result => {
    ...
    async addElement() {
      await this.addEstadoDemanda(this.form_data).then(result => {
    ...
    async editElement(aux) {
      await this.editEstadoDemanda(this.form_data).then(result => {
        if (result.success === true) {
          this.loadData()
    ...
    async deleteElement(aux) {
      await this.deleteEstadoDemanda(this.form_data).then(result => {
        if (result.success === true) {
          this.loadData()
    ...
    reset_form_modal() {
      this.form_data.desc_EstadoDemanda = '',
      this.action = ''
    },
    closeModal() {
