          ...
          <q-card-section class="no-padding">
            <q-card-section class="row no-padding">
              <q-toolbar class="bg-primary text-white">
                <q-toolbar-title>
                  {{
                      this.action == "add" ? "Crear Operadora" : this.action == "edit" ? "Editar Operadora" : "Eliminar Operadora"
                  }}
                </q-toolbar-title>
                <q-btn
                  round
            ...
            <q-card-section>
              <div class="row">
                <div class="col" style="padding: 0px 20px 20px 0px">
                  <q-input v-if="action != 'delete'" name="desc" v-model="form_data.desc_Operadora" type="text" label="Descripción" dense/>
                  <label v-if="action == 'delete'"  name="desc1"  type="text"  dense
                  >
                    Está seguro de eliminar la operadora: {{this.form_data.desc_Operadora}}</label>
                </div>
              </div>
              <br/>
...
import {getOperadoras} from "../../store/sistema_gestion/actions";


export default {
  name: 'sistemagestionoperadora',
  data() {
    return {
      data: [],
      columns: [
      ...
      visibleColumns: ['columnname','columnaction'],
      form_data: {
        id:'',
        desc_Operadora:'',
  ...
  methods: {
    ...
    ...mapActions('managementsystem', ['addOperadora']),
    ...mapActions('managementsystem', ['editOperadora']),
    ...mapActions('managementsystem', ['deleteOperadora']),
    async loadData() {
      this.loading = true
    ...
    async addElement() {
      await this.addOperadora(this.form_data).then(result => {
    ...
    async editElement(aux) {
      await this.editOperadora(this.form_data).then(result => {
        if (result.success === true) {
          this.loadData()
    ...
    async deleteElement(aux) {
      await this.deleteOperadora(this.form_data).then(result => {
        if (result.success === true) {
          this.loadData()
