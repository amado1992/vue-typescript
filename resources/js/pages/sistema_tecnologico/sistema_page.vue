<template>
  <div class="q-pa-md">
    ...
    <p>aDouble: {{ aDouble}}</p>
    <p>A: {{ a}}</p>
    <button v-on:click="incrementAplus">Increment aPlus</button>
    -->
    <!--
    <div>
   ...
   <!-- <q-input ref="sometext" filled v-model="someText" label="Some Text" />-->
...
export default {
  ...
  watch: {
   /* someText() {
    ...
    'form_create.nombre'(newVal, oldVal){
  ...
  methods: {
    ...
    confirmDelete(dataRow) {
      this.$q
        .dialog({
          title: "Confirme",
          message: "¿Estás seguro que desea eliminar este sistema?",
          cancel: true,
          persistent: true,
        })
        .onOk(() => {
    ...
    async save() {
      ...
      return await axios
        .post("/api/sistema", this.form_create)
        .then((result) => {
          this.loadData();
    ...
    async update() {
      ...
      return await axios
        .put("/api/sistema/" + id, this.form_create)
        .then((result) => {
          this.loadData();
