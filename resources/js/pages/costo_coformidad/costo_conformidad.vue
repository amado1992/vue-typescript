<template>
  <div class="q-pa-md">
    <q-toolbar class="bg-primary">
      ...
      <q-item-label class="text-white">Gestión de Costo de Conformidades</q-item-label>
      <q-space></q-space>
      ...
      <q-card style="width: 500px">
        <q-card-section class="bg-primary">
          <div class="text-h6 text-white">
            {{ this.action == 'add' ? 'Adicionar Costo de conformidad' : 'Actualizar Costo de conformidad' }}
          </div>
        </q-card-section>
    ...
    >
      <q-card style="width: 500px">
        <q-card-section class="bg-primary">
          <div class="text-h6 text-white">
            {{ this.action == 'see' ? 'Consultar Costo de conformidad' : 'Consultar Costo de conformidad' }}
          </div>
        </q-card-section>
        <q-card-section class="q-pa-lg">
          <div class="row">
...
export default {
  ...
  methods: {
    ...
    async nuevo(form_sistema) {
        return  await axios.post('/api/costo_conformidad', form_sistema)
            .then((result) => {
    ...
    async update(id, form_sitema) {
      ...
      async eliminar() {
          if (this.selected.length > 0) {
              ...
              return  await axios.delete('/api/costos_conformidades/' + id)
                  .then((result) => {
                      if(result.status === 200){
                          this.data = result
                           this.loadData()
