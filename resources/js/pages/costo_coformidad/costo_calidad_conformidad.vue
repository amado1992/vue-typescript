<template>
    <div class="q-pa-md">
        <q-toolbar class="bg-primary">
            ...
            <q-item-label class="text-white">Gestión de Costo Calidad Conformidad</q-item-label>
            <q-space></q-space>
                ...
                <q-card-section class="q-pa-lg">
                    <div class="row">
                        <div class="col" style="padding: 0px 20px 20px 0px">
                            <q-item-label>Conformidad: {{ conformidades}}</q-item-label>
...
export default {
    ...
    data() {
        return {
            visibleColumns: ['conformidad', 'reporte','importe','acumulado'],
            modalDialog: false,
            ...
            columns: [
                ...
                },
                {
                    ...
                    label: 'Conformidad',
                    align: 'center',
                    field: row => row.costo_conformidad.nombre,
                    search: true
                },
                {
                    name: 'reporte',
    ...
    methods: {
      ...
      async  openModalSee(data, action) {
       ...
       async buscarConformidades(){
           const id = this.form_sistema.conformidad_id;
            return await axios.get('/api/costos_conformidades/' + id)
                .then((result) => {
                    if (result.status === 200) {
                        this.conformidades = result.data.data.nombre
        ...
        async loadData() {
            return await axios.get('/api/costo_calidad_conformidades')
                .then((result) => {
        ...
        async loadDataConformidad() {
            return await axios.get('/api/costos_conformidades')
                .then((result) => {
                    this.conformidad = result.data.data
                }).catch(err => {
                    console.log(err)
                })
        },
        async nuevo(form_sistema) {
            return await axios.post('/api/costo_calidad_conformidad', form_sistema)
                .then((result) => {
        ...
        async update(id, form_sitema) {
            return await axios.put('/api/costo_calidad_conformidad/' + id, {data: form_sitema})
                .then((result) => {
                    if (result.status === 200) {
                        this.data = result
                        this.loadData()
        ...
        async eliminar() {
            if (this.selected.length > 0) {
                ...
                return await axios.delete('/api/costo_calidad_conformidad/' + id)
                    .then((result) => {
                        if (result.status === 200) {
                            this.data = result
                            this.loadData()
