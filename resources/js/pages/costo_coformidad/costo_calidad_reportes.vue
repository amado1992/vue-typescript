<template>
    <div class="q-pa-md">
        <q-toolbar class="bg-primary">
            ...
            <q-item-label class="text-white">Gestión de Costo Calidad Reportes</q-item-label>
            <q-space></q-space>
        ...
        >
            <q-card style="width: 500px">
                ...
                <q-card-section class="q-pa-lg">
                    ...
                    <div class="row">
                        <q-input v-model="form_sistema.fecha" filled type="date" />
                    </div>
                    <br/>
                    <div class="row">
                        <q-input outlined v-model="form_sistema.ventaNetaImporte" type="number" label="Venta Neta Importe*" prefix="$">
                        </q-input>

                        <q-input outlined v-model="form_sistema.ventaNetaAcumulada" type="number" label="Venta Neta Acumulada *" prefix="$" style="margin-left: 10px">
                        </q-input>
                    </div>
                    <br/>
                    <div class="row">
                        <div class="col-3 q-pr-sm">
                            <q-select
                                v-model="form_sistema.instalacion_id"
                                :options="instalacion"
                                label="Instalación"
                                options-dense
                                option-value="id"
        ...
        >
            <q-card style="width: 500px">
                ...
                <q-card-section class="q-pa-lg">
                    ...
                    <div class="row">
                        <div class="col" style="padding: 0px 20px 20px 0px">
                            <q-item-label>Fecha: {{ form_sistema.fecha }}</q-item-label>

                        </div>
                    </div>
                    <div class="row">
                        <div class="col" style="padding: 0px 20px 20px 0px">
                            <q-item-label>Venta Neta Importe: {{ form_sistema.ventaNetaImporte }}</q-item-label>

                        </div>
                    </div>
                    <div class="row">
                        <div class="col" style="padding: 0px 20px 20px 0px">
                            <q-item-label>Venta Neta Acumulada: {{ form_sistema.ventaNetaAcumulada }}</q-item-label>
...
export default {
    ...
    data() {
        return {
            visibleColumns: ['codigo', 'nombre','fecha','ventaNetaImporte','ventaNetaAcumulada'],
            modalDialog: false,
    ...
    methods: {
        ...
        async loadDataInstalacion() {
            return await axios.get('/api/get_instalacion')
                .then((result) => {
                    console.log(result.data)
                    this.instalacion = result.data
                }).catch(err => {
                    console.log(err)
                })
        },
        async nuevo(form_sistema) {
            return await axios.post('/api/costo_calidad_report', form_sistema)
                .then((result) => {
        ...
        async update(id, form_sitema) {
            return await axios.put('/api/costo_calidad_report/' + id, {data: form_sitema})
                .then((result) => {
        ...
        async eliminar() {
            if (this.selected.length > 0) {
                ...
                return await axios.delete('/api/costo_calidad_report/' + id)
                    .then((result) => {
                        if (result.status === 200) {
                            this.data = result
                            this.loadData()
