  ...
  <div class="q-pa-md">
    <q-card>
      ...
      <q-card-section>
        <q-table
          class="table-title"
          title="Gestionar cadena hotelera"
        ...
        >
          <!--<template v-slot:top>
            ...
            <q-btn
              key="filter"
              dense
              :disable="cadena_hotelera.length != 0 ? false : true"
            ...
            <q-card class="q-ma-xs">
              <q-card-section>
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteCadena_hoteleras(props.row)"
                >
      ...
      <q-card style="width: 600px; max-width: 70vw;">
        ...
        <q-card-section>
          ...
          <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 q-px-sm">
              <q-input
                v-model="form_data.cadena_hotelera"
                type="text"
                label="Nombre *"
                dense
                name="cadena_hotelera"
                v-if="form_data.activo === '1'"
                :error-message="mensajesError('cadena_hotelera')"
                :error="$v.form_data.cadena_hotelera.$error"
...
{ label: 'Cadena hotelera' },
  ...
  computed: {
    ...mapState('cadena_hotelera', ['cadena_hotelera']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('utils', ['setLoading']),
    ...mapActions('breadcrumbs', ['addToHomeBreadcrumbs']),
    ...mapActions('cadena_hotelera', ['listCadena_hotelera', 'saveCadena_hotelera', 'editCadena_hotelera', 'deleteCadena_hotelera']),
    mensajesError(campo) {
      if (!this.$v.form_data.$invalid) {
        this.required_error = false;
        this.form_error = false;
      }
      if (campo === 'cadena_hotelera') {
        if (!this.$v.form_data.cadena_hotelera.required) {
    ...
    reset_form() {
      ...
      this.form_data.cadena_hotelera = '';
      this.form_error = false;
      this.required_error = false;
    ...
    async loadData(props) {
      ...
      await this.listCadena_hotelera(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveCadena_hotelera(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editCadena_hotelera(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteCadena_hoteleras(selected) {
      if (selected) {
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteCadena_hotelera(selected.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
