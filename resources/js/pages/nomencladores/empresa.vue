        ...
        >
          <!--<template v-slot:top>
            ...
            <q-btn
              key="filter"
              dense
              :disable="empresa.length != 0 ? false : true"
            ...
            <q-card class="q-ma-xs">
              <q-card-section>
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteEmpresas(props.row)"
                >
      ...
      <q-card style="width: 600px; max-width: 70vw;">
        ...
        <q-card-section>
         ...
         <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 q-px-sm">
              <q-input
                v-model="form_data.empresa"
                type="text"
                label="Nombre *"
                dense
                name="empresa"
                v-if="form_data.activo === '1'"
                :error-message="mensajesError('empresa')"
                :error="$v.form_data.empresa.$error"
            ...
            <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 q-px-sm">
              <q-select
                v-model="form_data.tipo_empresa"
                :options="options"
                label="Tipo de empresa *"
                dense
                emit-value
                map-options
                :error-message="mensajesError('tipo_empresa')"
                :error="$v.form_data.tipo_empresa.$error"
...
import { required, alphaNum, email, numeric, maxLength, alpha } from 'vuelidate/lib/validators';
...
export default {
  ...
  data() {
    return {
      ...
      items_osde: [],
      options: ['Importadora', 'Exportadora'],
      modalDialog: false,
      filterInput: false,
      filter: '',
      accion: '',
      ...
      visibleColumns: [
        'activo', 'empresa', 'tipo_empresa', 'osde_id', 'direccion', 'correo', 'telefono', 'action'
      ],
      columns: [
        {
          name: 'activo',
        ...
        },
        {
          ...
          align: 'left',
          label: 'osde',
          field: (row) => row.osdes && row.osdes.nombre,
          headerClasses: 'text-uppercase',
        },
        {
          name: 'correo',
  ...
  methods: {
    ...
    ...mapActions('empresa', ['listEmpresa', 'saveEmpresa', 'editEmpresa', 'deleteEmpresa']),
    ...
    mensajesError(campo) {
      ...
      if (campo === 'empresa') {
        if (!this.$v.form_data.empresa.required) {
          this.required_error = true;
          return '';
        }
      }
      if (campo === 'tipo_empresa') {
        if (!this.$v.form_data.tipo_empresa.required) {
          this.required_error = true;
          return '';
        }
      }
    ...
    async loadData(props) {
      ...
      await this.listEmpresa(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveEmpresa(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editEmpresa(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteEmpresas(selected) {
      if (selected) {
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteEmpresa(selected.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
