  ...
  <div class="q-pa-md">
    <q-card>
      ...
      <q-card-section>
        <q-table
          class="table-title"
          title="Gestionar unidad técnica de inversiones del turismo"
        ...
        >
          <!--<template v-slot:top>
            ...
            <q-btn
              key="filter"
              dense
              :disable="utit.length != 0 ? false : true"
            ...
            <q-card class="q-ma-xs">
              <q-card-section>
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteUtits(props.row)"
                >
...
export default {
  ...
  data() {
    return {
      ...
      visibleColumns: ["activo", "codigo", "nombre", "action"],
      columns: [
        {
          name: "activo",
          align: "left",
  ...
  methods: {
    ...
    ...mapActions("utit", ["listUtit", "saveUtit", "editUtit", "deleteUtit"]),
    mensajesError(campo) {
    ...
    async loadData(props) {
      ...
      await this.listUtit(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveUtit(this.form_data).then((result) => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editUtit(this.form_data).then((result) => {
          if (result.error == 500) {
            warningMessage(result.message);
          } else {
            this.reset_form();
    ...
    deleteUtits(selected) {
      if (selected) {
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteUtit(selected.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
