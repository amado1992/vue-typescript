  ...
  <div class="q-pa-md">
    <q-card>
      ...
      <q-card-section>
        <q-table
          class="table-title"
          title="Gestionar categoría de instalación"
          :grid="xssmallScreen || smallScreen"
          flat
          dense
          :data="categoria_instalacion"
            ...
            <q-card class="q-ma-xs">
              <q-card-section>
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteCategoria_instalacions(props.row)"
                >
            ...
            <q-toolbar-title>
              {{
                this.accion == 'adicionar'
                  ? 'Adicionar categoría de instalación'
                  : 'Actualizar categoría de instalación'
              }}
            </q-toolbar-title>
        ...
        <q-card-section>
          <div class="row">
            ...
            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 q-px-sm">
              <q-input
                v-model="form_data.categoria_instalacion"
                type="text"
                label="Nombre *"
                name="categoria_instalacion"
                v-if="form_data.activo === '1'"
                :error-message="mensajesError('categoria_instalacion')"
                :error="$v.form_data.categoria_instalacion.$error"
...
export default {
  ...
  computed: {
    ...mapState('categoria_instalacion', ['categoria_instalacion']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('utils', ['setLoading']),
    ...mapActions('breadcrumbs', ['addToHomeBreadcrumbs']),
    ...mapActions('categoria_instalacion', ['listCategoria_instalacion', 'saveCategoria_instalacion', 'editCategoria_instalacion', 'deleteCategoria_instalacion']),
    mensajesError(campo) {
      if (!this.$v.form_data.$invalid) {
        this.required_error = false;
        this.form_error = false;
      }
      if (campo === 'categoria_instalacion') {
        if (!this.$v.form_data.categoria_instalacion.required) {
    ...
    reset_form() {
      ...
      this.form_data.categoria_instalacion = '';
      this.form_error = false;
      this.required_error = false;
    ...
    async loadData(props) {
      ...
      await this.listCategoria_instalacion(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveCategoria_instalacion(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editCategoria_instalacion(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteCategoria_instalacions(selected) {
      if (selected) {
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteCategoria_instalacion(selected.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
