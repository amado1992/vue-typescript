        ...
        >
          <template v-slot:top>
            ...
            <q-btn
              key="filter"
              dense
              :disable="indicador.length != 0 ? false : true"
      ...
      <q-card style="width: 600px; max-width: 70vw;">
        ...
        >
          <q-card-section class="no-padding">
            ...
            <q-card-section>
              <!--<q-scroll-area style="height:76px; max-width: 600px;">-->
              <div class="row q-gutter-y-xs">
                ...
                <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 q-px-sm">
                  <q-select
                    ...
                    emit-value
                    map-options
                    :error-message="mensajesError('renglon_id')"
                    :error="$v.form_data.renglon_id.$error"
  ...
  export default {
      ...
      return {
        ...
        columns: [
          ...
          },
          {
            ...
            align: 'left',
            label: 'Renglón',
            field: (row) => row.renglon && row.renglon.nombre,
            sortable: true,
            headerClasses: 'text-uppercase',
          },
          {
    ...
    methods: {
      ...
      mensajesError(campo) {
        ...
        if (campo === 'renglon_id') {
          if (!this.$v.form_data.renglon_id.required) {
            this.required_error = true;
            return '';
          }
        }
      ...
      async loadData(props) {
        ...
        await this.getIndicador(data).then((r) => {
      ...
      async save() {
        ...
        if (this.$v.form_data.$invalid) {
          ...
          this.setLoading(true);
          await this.saveIndicador(this.form_data).then(result => {
            if (result.error == 500) {
      ...
      async update() {
        ...
        if (this.$v.form_data.$invalid) {
          ...
          this.setLoading(true);
          await this.editIndicador(this.form_data).then(result => {
            if (result.error == 500) {
              warningMessage(result.message);
            }else{
              this.reset_form();
      ...
      deleteIndicadores(selected) {
        if (selected) {
          ...
          showDialog(message.lAskForRemove).onOk(async () => {
            this.setLoading(true);
            await this.deleteIndicador(this.form_data.id);
            this.reset_form();
            await this.loadData({
              pagination: this.pagination,
              filter: this.filter,
