        ...
        >
          <!--<template v-slot:top>
            ...
            <q-btn
              key="filter"
              dense
              :disable="escuela_ramal.length != 0 ? false : true"
            ...
            <q-card class="q-ma-xs">
              <q-card-section>
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteEscuela_ramals(props.row)"
                >
...
export default {
  ...
  computed: {
    ...mapState('escuela_ramal', ['escuela_ramal']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('utils', ['setLoading']),
    ...mapActions('breadcrumbs', ['addToHomeBreadcrumbs']),
    ...mapActions('escuela_ramal', ['listEscuela_ramal', 'saveEscuela_ramal', 'editEscuela_ramal', 'deleteEscuela_ramal']),
    mensajesError(campo) {
    ...
    async loadData(props) {
      ...
      await this.listEscuela_ramal(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveEscuela_ramal(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editEscuela_ramal(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteEscuela_ramals(selected) {
      if (selected) {
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteEscuela_ramal(selected.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
