        ...
        >
         <!-- <template v-slot:top>
            ...
            <q-btn
              key="filter"
              dense
              :disable="sucursal.length != 0 ? false : true"
            ...
            <q-card class="q-ma-xs">
              <q-card-section>
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteSucursals(props.row)"
                >
        ...
        <q-card-section>
          <div class="row">
            ...
            <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 q-px-sm">
              <q-input
                v-model="form_data.sucursal"
                type="text"
                label="Nombre *"
                dense
                name="sucursal"
                v-if="form_data.activo === '1'"
                :error-message="mensajesError('sucursal')"
                :error="$v.form_data.sucursal.$error"
...
import { required, alphaNum, numeric, email, maxLength } from 'vuelidate/lib/validators';
...
export default {
  ...
  data() {
    return {
      ...
      visibleColumns: [
        'activo', 'sucursal','complejo', 'empresa_id', 'direccion', 'correo', 'telefono', 'action'
      ],
      columns: [
        {
          name: 'activo',
  ...
  computed: {
    ...mapState('sucursal', ['sucursal']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('utils', ['setLoading']),
    ...mapActions('breadcrumbs', ['addToHomeBreadcrumbs']),
    ...mapActions('sucursal', ['listSucursal', 'saveSucursal', 'editSucursal', 'deleteSucursal']),
    ...
    mensajesError(campo) {
      ...
      if (campo === 'sucursal') {
        if (!this.$v.form_data.sucursal.required) {
    ...
    async loadData(props) {
      ...
      await this.listSucursal(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveSucursal(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editSucursal(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteSucursals(selected) {
      if (selected) {
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteSucursal(selected.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
