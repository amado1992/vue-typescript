  ...
  <div class="q-pa-md">
    <q-card>
      ...
      <q-card-section>
        <q-table
          class="table-title"
          title="Gestionar oficina de empleo"
        ...
        >
          <!--<template v-slot:top>
            ...
            <q-btn
              key="filter"
              dense
              :disable="oficina_empleo.length != 0 ? false : true"
            ...
            <q-card class="q-ma-xs">
              <q-card-section>
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteOficina_empleos(props.row)"
                >
      ...
      <q-card style="width: 600px; max-width: 70vw;">
            ...
            <q-toolbar-title>
              {{
                this.accion == 'adicionar'
                  ? 'Adicionar oficina de empleo'
                  : 'Actualizar oficina de empleo'
              }}
            </q-toolbar-title>
        ...
        <q-card-section>
          ...
          <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 q-px-sm">
              <q-input
                v-model="form_data.oficina_empleo"
                type="text"
                label="Nombre *"
                dense
                name="oficina_empleo"
                v-if="form_data.activo === '1'"
                :error-message="mensajesError('oficina_empleo')"
                :error="$v.form_data.oficina_empleo.$error"
...
export default {
  ...
  data() {
    return {
      ...
      visibleColumns: [
        'activo', 'oficina_empleo', 'delegacion_territorial_id', 'direccion', 'telefono', 'action'
      ],
      columns: [
        {
          name: 'activo',
  ...
  computed: {
    ...mapState('oficina_empleo', ['oficina_empleo']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('utils', ['setLoading']),
    ...mapActions('breadcrumbs', ['addToHomeBreadcrumbs']),
    ...mapActions('oficina_empleo', ['listOficina_empleo', 'saveOficina_empleo', 'editOficina_empleo', 'deleteOficina_empleo']),
    ...
    mensajesError(campo) {
      ...
      if (campo === 'oficina_empleo') {
        if (!this.$v.form_data.oficina_empleo.required) {
    ...
    async loadData(props) {
      ...
      await this.listOficina_empleo(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveOficina_empleo(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editOficina_empleo(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteOficina_empleos(selected) {
      if (selected) {
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteOficina_empleo(selected.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
