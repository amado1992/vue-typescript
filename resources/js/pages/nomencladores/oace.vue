        ...
        >
          <!--<template v-slot:top>
            ...
            <q-btn
              key="filter"
              dense
              :disable="oace.length != 0 ? false : true"
            ...
            <q-card class="q-ma-xs">
              <q-card-section>
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteOaces(props.row)"
                >
      ...
      <q-card style="width: 600px; max-width: 70vw;">
        ...
        <q-card-section>
          ...
          <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 q-px-sm">
              <q-input
                v-model="form_data.siglas"
                type="text"
                label="Siglas *"
                dense
                name="siglas"
                v-if="form_data.activo === '1'"
                :error-message="mensajesError('siglas')"
                :error="$v.form_data.siglas.$error"
           ...
           <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 q-px-sm">
              <q-input
                v-model="form_data.oace"
                type="text"
                label="Nombre *"
                dense
                name="oace"
                v-if="form_data.activo === '1'"
                :error-message="mensajesError('oace')"
                :error="$v.form_data.oace.$error"
                :rules="[val => !!val || 'El campo es requerido', isValidAlphaNum]"
                debounce="1000"
                autogrow
              />
...
export default {
  ...
  methods: {
    ...
    ...mapActions('oace', ['listOace', 'saveOace', 'editOace', 'deleteOace']),
    mensajesError(campo) {
      ...
      if (campo === 'siglas') {
        if (!this.$v.form_data.siglas.required) {
          this.required_error = true;
          return '';
        }
      }
      if (campo === 'oace') {
        if (!this.$v.form_data.oace.required) {
          this.required_error = true;
          return '';
        }
      }
    ...
    async loadData(props) {
      ...
      await this.listOace(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveOace(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editOace(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteOaces(selected) {
      if (selected) {
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteOace(selected.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
