  ...
  <div class="q-pa-md">
    <q-card>
      ...
      <q-card-section>
        <q-table
          class="table-title"
          title="Gestionar delegacion territorial"
          :grid="xssmallScreen || smallScreen"
          flat
          dense
          :data="delegacion_territorial"
        ...
        >
          <!--<template v-slot:top>
            <div class="text-subtitle1 text-weight-bold text-uppercase">
              Gestionar delegacion territorial
            </div>
            <q-space></q-space>
            <q-input
              dense
            ...
            <q-btn
              key="filter"
              dense
              :disable="delegacion_territorial.length != 0 ? false : true"
            ...
            <q-card class="q-ma-xs">
              <q-card-section>
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteDelegacion_territorials(props.row)"
                >
      ...
      <q-card style="width: 600px; max-width: 70vw;">
            ...
            <q-toolbar-title>
              {{
                this.accion == 'adicionar'
                  ? 'Adicionar delegacion territorial'
                  : 'Actualizar delegacion territorial'
              }}
            </q-toolbar-title>
        ...
        <q-card-section>
          ...
          <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 q-px-sm">
              <q-input
                v-model="form_data.delegacion_territorial"
                type="text"
                label="Nombre *"
                dense
                name="delegacion_territorial"
                v-if="form_data.activo === '1'"
                :error-message="mensajesError('delegacion_territorial')"
                :error="$v.form_data.delegacion_territorial.$error"
            ...
            <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 q-px-sm">
              <q-select
                v-model="form_data.territorio_id"
                dense
                emit-value
                map-options
                options-dense
                :options="items_territorio"
                label="Territorio *"
                option-label="nombre"
                option-value="id"
                v-if="form_data.activo === '1'"
                :error-message="mensajesError('territorio_id')"
                :error="$v.form_data.territorio_id.$error"
...
export default {
  ...
  data() {
    return {
      ...
      visibleColumns: [
        'activo', 'delegacion_territorial', 'territorio_id', 'action'
      ],
      columns: [
        {
          name: 'activo',
        ...
        },
        {
          name: 'delegacion_territorial',
          align: 'left',
          label: 'delegacion territorial',
          field: 'delegacion_territorial',
          headerClasses: 'text-uppercase',
        },
        {
          name: 'territorio_id',
          align: 'left',
          label: 'territorio',
          field: (row) => row.provincias && row.provincias.nombre,
          headerClasses: 'text-uppercase',
        },
        {
          name: 'action',
  ...
  computed: {
    ...mapState('delegacion_territorial', ['delegacion_territorial']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('utils', ['setLoading']),
    ...mapActions('breadcrumbs', ['addToHomeBreadcrumbs']),
    ...mapActions('delegacion_territorial', ['listDelegacion_territorial', 'saveDelegacion_territorial', 'editDelegacion_territorial', 'deleteDelegacion_territorial']),
    ...
    mensajesError(campo) {
      ...
      if (campo === 'delegacion_territorial') {
        if (!this.$v.form_data.delegacion_territorial.required) {
          this.required_error = true;
          return '';
        }
      }
      if (campo === 'territorio_id') {
        if (!this.$v.form_data.territorio_id.required) {
          this.required_error = true;
          return '';
        }
      }
    ...
    reset_form() {
      ...
      this.form_data.delegacion_territorial = '';
      this.form_data.territorio_id = '';
      this.form_error = false;
      this.required_error = false;
    ...
    async loadData(props) {
      ...
      await this.listDelegacion_territorial(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveDelegacion_territorial(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editDelegacion_territorial(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteDelegacion_territorials(selected) {
      if (selected) {
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteDelegacion_territorial(selected.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
