        ...
        >
          <!--<template v-slot:top>
            ...
            <q-btn
              key="filter"
              dense
              :disable="infotur.length != 0 ? false : true"
            ...
            <q-card class="q-ma-xs">
              <q-card-section>
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteInfoturs(props.row)"
                >
      ...
      <q-card style="width: 600px; max-width: 70vw;">
        ...
        <q-card-section>
          ...
          <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 q-px-sm">
              <q-input
                v-model="form_data.infotur"
                type="text"
                label="Nombre *"
                dense
                name="infotur"
                v-if="form_data.activo === '1'"
                :error-message="mensajesError('infotur')"
                :error="$v.form_data.infotur.$error"
...
export default {
  ...
  data() {
    return {
      ...
      visibleColumns: [
        'activo', 'infotur', 'delegacion_territorial_id', 'direccion', 'telefono', 'action'
      ],
      columns: [
        {
          name: 'activo',
  ...
  computed: {
    ...mapState('infotur', ['infotur']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('utils', ['setLoading']),
    ...mapActions('breadcrumbs', ['addToHomeBreadcrumbs']),
    ...mapActions('infotur', ['listInfotur', 'saveInfotur', 'editInfotur', 'deleteInfotur']),
    ...
    mensajesError(campo) {
      ...
      if (campo === 'infotur') {
        if (!this.$v.form_data.infotur.required) {
    ...
    async loadData(props) {
      ...
      await this.listInfotur(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveInfotur(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editInfotur(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteInfoturs(selected) {
      if (selected) {
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteInfotur(selected.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
