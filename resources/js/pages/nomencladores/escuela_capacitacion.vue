  ...
  <div class="q-pa-md">
    <q-card>
      ...
      <q-card-section>
        <q-table
          class="table-title"
          title="Gestionar escuela de capacitación"
          :grid="xssmallScreen || smallScreen"
          flat
          dense
          :data="escuela_capacitacion"
        ...
        >
          <!--<template v-slot:top>
            <div class="text-subtitle1 text-weight-bold text-uppercase">
              Gestionar escuela de capacitación
            </div>
            <q-space></q-space>
            <q-input
              dense
            ...
            <q-btn
              key="filter"
              dense
              :disable="escuela_capacitacion.length != 0 ? false : true"
            ...
            <q-card class="q-ma-xs">
              <q-card-section>
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteEscuela_capacitacions(props.row)"
                >
            ...
            <q-toolbar-title>
              {{
                this.accion == 'adicionar'
                  ? 'Adicionar escuela de capacitación'
                  : 'Actualizar escuela de capacitación'
              }}
            </q-toolbar-title>
        ...
        <q-card-section class="q-px-md">
          <div class="row">
            ...
            <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 q-px-sm">
              <q-input
                v-model="form_data.escuela_capacitacion"
                type="text"
                label="Nombre *"
                dense
                name="escuela_capacitacion"
                v-if="form_data.activo === '1'"
                :error-message="mensajesError('escuela_capacitacion')"
                :error="$v.form_data.escuela_capacitacion.$error"
            ...
            <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 q-px-sm">
              <q-select
                v-model="form_data.delegacion_territorial_id"
                dense
                emit-value
                map-options
                options-dense
                :options="items_delegacion_territorial"
                label="Delegación territorial *"
                option-label="delegacion_territorial"
                option-value="id"
                v-if="form_data.activo === '1'"
                :error-message="mensajesError('delegacion_territorial_id')"
                :error="$v.form_data.delegacion_territorial_id.$error"
...
export default {
  ...
  data() {
    return {
      ...
      visibleColumns: [
        'activo', 'escuela_capacitacion', 'delegacion_territorial_id', 'direccion', 'telefono', 'action'
      ],
      columns: [
        {
          name: 'activo',
        ...
        {
          name: 'delegacion_territorial_id',
          align: 'left',
          label: 'delegación territorial',
          field: (row) => row.delegacionesterritoriales && row.delegacionesterritoriales.delegacion_territorial,
          headerClasses: 'text-uppercase',
        },
        {
          name: 'escuela_capacitacion',
          align: 'left',
          label: 'escuela de capacitación',
          field: 'escuela_capacitacion',
          headerClasses: 'text-uppercase',
        },
        {
          name: 'direccion',
  ...
  mounted() {
    ...
    this.getDelegacion_territorial().then((r) => {
      this.items_delegacion_territorial = r;
    });
  },
  watch: {
    pagination: {
  ...
  computed: {
    ...mapState('escuela_capacitacion', ['escuela_capacitacion']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('utils', ['setLoading']),
    ...mapActions('breadcrumbs', ['addToHomeBreadcrumbs']),
    ...mapActions('escuela_capacitacion', ['listEscuela_capacitacion', 'saveEscuela_capacitacion', 'editEscuela_capacitacion', 'deleteEscuela_capacitacion']),
    ...
    mensajesError(campo) {
      ...
      if (campo === 'escuela_capacitacion') {
        if (!this.$v.form_data.escuela_capacitacion.required) {
          this.required_error = true;
          return '';
        }
      }
      if (campo === 'delegacion_territorial_id') {
        if (!this.$v.form_data.delegacion_territorial_id.required) {
          this.required_error = true;
          return '';
        }
      }
    ...
    reset_form() {
      ...
      this.form_data.escuela_capacitacion = '';
      this.form_data.delegacion_territorial_id = '';
    ...
    async loadData(props) {
      ...
      await this.listEscuela_capacitacion(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveEscuela_capacitacion(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editEscuela_capacitacion(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
  ...
  deleteEscuela_capacitacions(selected) {
      if (selected) {
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteEscuela_capacitacion(selected.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
