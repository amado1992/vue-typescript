...
export default {
  ...
  data() {
    return {
      ...
      chartExample: {
        ...
        xAxis: {
          categories: [
            'Total de costos de conformidad',
            'Total de costos de no conformidad',
        ...
        series: [
          ...
          {
            name: 'Igual trimestre del año anterior',
            data: [],
          },
        ],
      },
  ...
  mounted() {
    this.addToHomeBreadcrumbs([
      ...
      { label: 'Reportes' },
      { label: 'Filtrar totales de costos' },
    ]);
  ...
  methods: {
    ...
    async loadDataAnnoActual() {
      ...
      await this.getYearEntiyAllCostTotalPage_report(
        this.parametersDatePicker
      ).then((response) => {
        this.chartExample.series[0].data.push(
          parseFloat(response.total_costo_conformidad)
        );
        this.chartExample.series[0].data.push(
          parseFloat(response.total_costo_noconformidad)
        );
        this.chartExample.series[0].data.push(
          parseFloat(response.total_costo_calidad)
        );
        this.chartExample.series[0].data.push(
          parseFloat(response.total_venta_neta)
        );
      });
    },
