        ...
        >
          <template v-slot:top>
            ...
            <q-btn
              key="filter"
              dense
              :disable="deteccion.length != 0 ? false : true"
          ...
          <div class="row">
            <div class="col-12">
              <q-input
                ...
                dense
                name="deteccion"
                :error-message="mensajesError('deteccion')"
                :error="$v.form_data.deteccion.$error"
                debounce="1000"
                autogrow
              />
            </div>
...
export default {
  ...
  computed: {
    ...mapState('deteccion', ['deteccion']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('utils', ['setLoading']),
    ...mapActions('breadcrumbs', ['addToHomeBreadcrumbs']),
    ...mapActions('deteccion', ['listDeteccion', 'saveDeteccion', 'editDeteccion', 'deleteDeteccion']),
    mensajesError(campo) {
      ...
      if (campo === 'deteccion') {
        if (!this.$v.form_data.deteccion.required) {
          this.required_error = true;
          return '';
        }
        if (!this.$v.form_data.deteccion.alpha) {
          this.required_error = true;
    ...
    async loadData(props) {
      ...
      await this.listDeteccion(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveDeteccion(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editDeteccion(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteDetecciones(selected) {
      if (selected) {
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteDeteccion(selected.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
