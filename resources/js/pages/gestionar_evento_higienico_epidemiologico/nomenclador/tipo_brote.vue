        ...
        >
          <template v-slot:top>
            ...
            <q-btn
              key="filter"
              dense
              :disable="tipo_brote.length != 0 ? false : true"
            ...
            <q-td :props="props">
              <div class="q-gutter-xs">
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteTipo_brotes(props.row)"
                >
            ...
            <q-toolbar-title>
              {{
                this.accion == 'adicionar'
                  ? 'Adicionar tipo de brote'
                  : 'Actualizar tipo de brote'
              }}
            </q-toolbar-title>
          ...
          <div class="row">
            <div class="col-12">
              <q-input
                ...
                dense
                name="tipo_brote"
                :error-message="mensajesError('tipo_brote')"
                :error="$v.form_data.tipo_brote.$error"
                debounce="1000"
                autogrow
              />
            </div>
...
export default {
  ...
  computed: {
    ...mapState('tipo_brote', ['tipo_brote']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('utils', ['setLoading']),
    ...mapActions('breadcrumbs', ['addToHomeBreadcrumbs']),
    ...mapActions('tipo_brote', ['listTipo_brote', 'saveTipo_brote', 'editTipo_brote', 'deleteTipo_brote']),
    mensajesError(campo) {
      ...
      if (campo === 'tipo_brote') {
        if (!this.$v.form_data.tipo_brote.required) {
          this.required_error = true;
          return '';
        }
        if (!this.$v.form_data.tipo_brote.alpha) {
          this.required_error = true;
    ...
    async loadData(props) {
      ...
      await this.listTipo_brote(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveTipo_brote(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editTipo_brote(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteTipo_brotes(selected) {
      if (selected) {
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteTipo_brote(selected.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
