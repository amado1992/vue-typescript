        ...
        >
          <template v-slot:top>
            ...
            <q-btn
              key="filter"
              dense
              :disable="origen_brote.length != 0 ? false : true"
            ...
            <q-td :props="props">
              <div class="q-gutter-xs">
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteOrigen_brotes(props.row)"
                >
            ...
            <q-toolbar-title>
              {{
                this.accion == 'adicionar'
                  ? 'Adicionar origen de brote'
                  : 'Actualizar origen de brote'
              }}
            </q-toolbar-title>
          ...
          <div class="row">
            <div class="col-12">
              <q-input
                ...
                dense
                name="origen_brote"
                :error-message="mensajesError('origen_brote')"
                :error="$v.form_data.origen_brote.$error"
                debounce="1000"
                autogrow
              />
            </div>
...
export default {
  ...
  computed: {
    ...mapState('origen_brote', ['origen_brote']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('utils', ['setLoading']),
    ...mapActions('breadcrumbs', ['addToHomeBreadcrumbs']),
    ...mapActions('origen_brote', ['listOrigen_brote', 'saveOrigen_brote', 'editOrigen_brote', 'deleteOrigen_brote']),
    mensajesError(campo) {
      if (!this.$v.form_data.$invalid) {
        this.required_error = false;
        this.form_error = false;
      }
      if (campo === 'origen_brote') {
        if (!this.$v.form_data.origen_brote.required) {
          this.required_error = true;
          return '';
        }
        if (!this.$v.form_data.origen_brote.alpha) {
          this.required_error = true;
    ...
    async loadData(props) {
      ...
      await this.listOrigen_brote(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveOrigen_brote(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editOrigen_brote(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteOrigen_brotes(selected) {
      if (selected) {
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteOrigen_brote(selected.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
