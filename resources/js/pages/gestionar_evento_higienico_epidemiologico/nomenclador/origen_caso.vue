        ...
        >
          <template v-slot:top>
            ...
            <q-btn
              key="filter"
              dense
              :disable="origen_caso.length != 0 ? false : true"
            ...
            <q-td :props="props">
              <div class="q-gutter-xs">
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteOrigen_casos(props.row)"
                >
            ...
            <q-toolbar-title>
              {{
                this.accion == 'adicionar'
                  ? 'Adicionar origen de caso'
                  : 'Actualizar origen de caso'
              }}
            </q-toolbar-title>
          ...
          <div class="row">
            <div class="col-12">
              <q-input
                ...
                type="text"
                label="Origen de caso *"
                dense
                name="origen_caso"
                :error-message="mensajesError('origen_caso')"
                :error="$v.form_data.origen_caso.$error"
                debounce="1000"
                autogrow
              />
            </div>
...
export default {
  ...
  computed: {
    ...mapState('origen_caso', ['origen_caso']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('utils', ['setLoading']),
    ...mapActions('breadcrumbs', ['addToHomeBreadcrumbs']),
    ...mapActions('origen_caso', ['listOrigen_caso', 'saveOrigen_caso', 'editOrigen_caso', 'deleteOrigen_caso']),
    mensajesError(campo) {
      if (!this.$v.form_data.$invalid) {
        this.required_error = false;
        this.form_error = false;
      }
      if (campo === 'origen_caso') {
        if (!this.$v.form_data.origen_caso.required) {
          this.required_error = true;
          return '';
        }
        if (!this.$v.form_data.origen_caso.alpha) {
          this.required_error = true;
    ...
    async loadData(props) {
      ...
      await this.listOrigen_caso(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveOrigen_caso(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editOrigen_caso(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteOrigen_casos(selected) {
      if (selected) {
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteOrigen_caso(selected.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
