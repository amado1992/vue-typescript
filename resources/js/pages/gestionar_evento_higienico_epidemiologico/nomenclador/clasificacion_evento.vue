        ...
        >
          <template v-slot:top>
            <div class="text-subtitle1 text-weight-bold text-uppercase">
              Gestionar clasificación de evento
            </div>
            <q-space></q-space>
            <q-input
              dense
            ...
            <q-btn
              key="filter"
              dense
              :disable="clasificacion_evento.length != 0 ? false : true"
            ...
            <q-td :props="props">
              <div class="q-gutter-xs">
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteTipo_proveedores(props.row)"
                >
            ...
            <q-toolbar-title>
              {{
                this.accion == 'adicionar'
                  ? 'Adicionar clasificación de evento'
                  : 'Actualizar clasificación de evento'
              }}
            </q-toolbar-title>
          ...
          <div class="row">
            <div class="col-12">
              <q-input
                ...
                :error-message="mensajesError('clasificacion_evento')"
                :error="$v.form_data.clasificacion_evento.$error"
                debounce="1000"
                autogrow
              />
            </div>
...
export default {
  name: 'clasificacion_eventos',
  data() {
    return {
      visibleColumns: ['codigo', 'clasificacion_evento', 'action'],
      modalDialog: false,
      filterInput: false,
      permiso: true,
      val: false,
  ...
  computed: {
    ...mapState('clasificacion_evento', ['clasificacion_evento']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('usuario', ['getTotalUsuariosRol']),
    ...mapActions('clasificacion_evento', [
      'listClasificacion_evento',
      'saveClasificacion_evento',
      'editClasificacion_evento',
      'deleteClasificacion_evento',
    ]),
    ...
    mensajesError(campo) {
      ...
      if (campo === 'clasificacion_evento') {
        if (!this.$v.form_data.clasificacion_evento.required) {
          this.required_error = true;
          return '';
        }
        if (!this.$v.form_data.clasificacion_evento.alpha) {
          this.required_error = true;
          return 'Solo se aceptan caracteres alfabéticos';
        }
        if (!this.$v.form_data.clasificacion_evento.maxLength) {
          this.required_error = false;
          return 'La clasificación de evento solo puede tener 30 carácteres';
        }
      }
    },
    filterFn(val, update) {
    ...
    reset_form() {
      this.form_data.clasificacion_evento = '';
      this.form_error = false;
      this.val = false;
      this.required_error = false;
      this.permiso = true;
    ...
    async loadData(props) {
      ...
      await this.listClasificacion_evento(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveClasificacion_evento(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editClasificacion_evento(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteClasificacion_eventos(selected) {
      if (selected) {
        this.setFormData(selected);
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteClasificacion_evento(this.form_data.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
