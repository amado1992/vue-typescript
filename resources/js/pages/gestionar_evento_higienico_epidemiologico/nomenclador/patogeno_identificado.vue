        ...
        >
          <template v-slot:top>
            <div class="text-subtitle1 text-weight-bold text-uppercase">
              Gestionar patógeno identificado
            </div>
            <q-space></q-space>
            <q-input
              dense
            ...
            <q-btn
              key="filter"
              dense
              :disable="patogeno_identificado.length != 0 ? false : true"
            ...
            <q-td :props="props">
              <div class="q-gutter-xs">
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deletePatogeno_identificados(props.row)"
                >
      ...
      <q-card style="width: 600px; max-width: 70vw;">
            ...
            <q-toolbar-title>
              {{
                this.accion == 'adicionar'
                  ? 'Adicionar patógeno identificado'
                  : 'Actualizar patógeno identificado'
              }}
            </q-toolbar-title>
        ...
        <q-card-section class="q-px-md">
          <div class="row">
            <div class="col-12">
              <q-input
                v-model="form_data.patogeno_identificado"
                type="text"
                label="Patógeno identificado *"
                dense
                name="nombre"
                :error-message="mensajesError('patogeno_identificado')"
                :error="$v.form_data.patogeno_identificado.$error"
                debounce="1000"
                autogrow
              />
            </div>
...
export default {
  name: 'patogeno_identificados',
  data() {
    return {
      visibleColumns: ['codigo', 'patogeno_identificado', 'action'],
      modalDialog: false,
      filterInput: false,
      permiso: true,
      val: false,
  ...
  computed: {
    ...mapState('patogeno_identificado', ['patogeno_identificado']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('usuario', ['getTotalUsuariosRol']),
    ...mapActions('patogeno_identificado', [
      'listPatogeno_identificado',
      'savePatogeno_identificado',
      'editPatogeno_identificado',
      'deletePatogeno_identificado',
    ]),
    ...
    mensajesError(campo) {
      ...
      if (campo === 'patogeno_identificado') {
        if (!this.$v.form_data.patogeno_identificado.required) {
          this.required_error = true;
          return '';
        }
        if (!this.$v.form_data.patogeno_identificado.alpha) {
          this.required_error = true;
          return 'Solo se aceptan caracteres alfabéticos';
        }
        if (!this.$v.form_data.patogeno_identificado.maxLength) {
          this.required_error = false;
          return 'El patógeno identificado solo puede tener 30 carácteres';
        }
      }
    },
    filterFn(val, update) {
    ...
    reset_form() {
      this.form_data.patogeno_identificado = '';
      this.form_error = false;
      this.val = false;
      this.required_error = false;
      this.permiso = true;
    ...
    async loadData(props) {
      ...
      await this.listPatogeno_identificado(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.savePatogeno_identificado(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editPatogeno_identificado(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deletePatogeno_identificados(selected) {
      if (selected) {
        this.setFormData(selected);
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deletePatogeno_identificado(this.form_data.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
