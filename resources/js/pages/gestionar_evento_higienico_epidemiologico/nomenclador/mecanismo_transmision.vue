        ...
        >
          <template v-slot:top>
            <div class="text-subtitle1 text-weight-bold text-uppercase">
              Gestionar mecanismo de transmisión
            </div>
            <q-space></q-space>
            <q-input
              dense
            ...
            <q-btn
              key="filter"
              dense
              :disable="mecanismo_transmision.length != 0 ? false : true"
            ...
            <q-td :props="props">
              <div class="q-gutter-xs">
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteMecanismo_transmisions(props.row)"
                >
      ...
      <q-card style="width: 600px; max-width: 70vw;">
            ...
            <q-toolbar-title>
              {{
                this.accion == 'adicionar'
                  ? 'Adicionar mecanismo de transmisión'
                  : 'Actualizar mecanismo de transmisión'
              }}
            </q-toolbar-title>
        ...
        <q-card-section class="q-px-md">
          <div class="row">
            <div class="col-12">
              <q-input
                v-model="form_data.mecanismo_transmision"
                type="text"
                label="Mecanismo de transmisión *"
                dense
                name="mecanismo_transmision"
                :error-message="mensajesError('mecanismo_transmision')"
                :error="$v.form_data.mecanismo_transmision.$error"
                debounce="1000"
                autogrow
              />
            </div>
...
export default {
  ...
  computed: {
    ...mapState('mecanismo_transmision', ['mecanismo_transmision']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('utils', ['setLoading']),
    ...mapActions('breadcrumbs', ['addToHomeBreadcrumbs']),
    ...mapActions('mecanismo_transmision', ['listMecanismo_transmision', 'saveMecanismo_transmision', 'editMecanismo_transmision', 'deleteMecanismo_transmision']),
    mensajesError(campo) {
      if (!this.$v.form_data.$invalid) {
        this.required_error = false;
        this.form_error = false;
      }
      if (campo === 'mecanismo_transmision') {
        if (!this.$v.form_data.mecanismo_transmision.required) {
          this.required_error = true;
          return '';
        }
        if (!this.$v.form_data.mecanismo_transmision.alpha) {
          this.required_error = true;
    ...
    reset_form() {
      this.form_data.mecanismo_transmision = '';
      this.form_error = false;
      this.required_error = false;
    ...
    async loadData(props) {
      ...
      await this.listMecanismo_transmision(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveMecanismo_transmision(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editMecanismo_transmision(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteMecanismo_transmisions(selected) {
      if (selected) {
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteMecanismo_transmision(selected.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
