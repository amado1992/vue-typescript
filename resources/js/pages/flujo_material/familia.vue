        ...
        >
          <template v-slot:top>
            <div class="text-subtitle1 text-weight-bold text-uppercase">
              Gestionar familia de productos
            </div>
            <q-space></q-space>
            <q-input
              dense
            ...
            <q-btn
              key="filter"
              dense
              :disable="familia.length != 0 ? false : true"
            ...
            <q-td :props="props">
              <div class="q-gutter-sm">
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteFamilias(props.row)"
                >
      ...
      <q-card style="width: 600px; max-width: 70vw;">
        ...
        <q-card-section>
          <div class="row q-px-sm">
            <div class="col-12">
              <q-input
                v-model="form_data.nombre_familia"
                type="text"
                label="Nombre *"
                dense
                name="nombre_familia"
                :error-message="mensajesError('nombre_familia')"
                :error="$v.form_data.nombre_familia.$error"
                debounce="1000"
                autogrow
              />
            </div>
...
export default {
  ...
  data() {
    return {
      visibleColumns: ['nombre_familia', 'descripcion', 'action'],
      modalDialog: false,
      filterInput: false,
      permiso: true,
      val: false,
  ...
  methods: {
    ...
    mensajesError(campo) {
      ...
      if (campo === 'nombre_familia') {
        if (!this.$v.form_data.nombre_familia.required) {
          this.required_error = true;
          return '';
        }
        if (!this.$v.form_data.nombre_familia.alpha) {
          this.required_error = true;
          return 'Solo se aceptan caracteres alfabéticos';
        }
        if (!this.$v.form_data.nombre_familia.maxLength) {
          this.required_error = false;
    ...
    typeSelection() {
      return this.scopes.includes('update_familia') ||
        this.scopes.includes('delete_familia')
        ? 'single'
        : 'none';
    },
    filterFn(val, update) {
    ...
    async loadData(props) {
      ...
      await this.getFamilia(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveFamilia(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editFamilia(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
