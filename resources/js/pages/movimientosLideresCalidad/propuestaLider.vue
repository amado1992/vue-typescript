        ...
        >
          <template v-slot:top>
            <div class="text-subtitle1 text-weight-bold text-uppercase">
              Gestionar propuesta de líderes
            </div>
            <q-space></q-space>
            <q-input
              dense
            ...
            <q-btn
              key="filter"
              dense
              :disable="propuesta.length != 0 ? false : true"
            ...
            <q-td :props="props">
              <div class="q-gutter-xs">
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deletePropuestas(props.row)"
                >
            ...
            <div class="row">
              ...
              <div class="col-4 q-px-sm">
                <q-input
                  ...
                  label="Apellidos *"
                  name="apellido"
                  :error-message="mensajesError('apellido')"
                  :error="$v.form_data.apellido.$error"
                  debounce="1000"
                />
              </div>
              <div class="col-4">
                <q-input
                  v-model="form_data.cargo"
                  type="text"
                  label="Cargo *"
                  name="cargo"
                  :error-message="mensajesError('cargo')"
                  :error="$v.form_data.cargo.$error"
                  debounce="1000"
                />
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    mensajesError(campo) {
      ...
      if (campo === 'argumentacion') {
        ...
        if (!this.$v.form_data.argumentacion.maxLength) {
          this.required_error = false;
          return 'El argumentación solo puede tener 30 carácteres';
        }
      }
      if (campo === 'nombre') {
      ...
      if (campo === 'apellido') {
        if (!this.$v.form_data.apellido.required) {
          this.required_error = true;
          return '';
        }
        if (!this.$v.form_data.apellido.maxLength) {
          this.required_error = false;
          return 'Los apellidos solo puede tener 30 carácteres';
        }
      }
      if (campo === 'cargo') {
        if (!this.$v.form_data.cargo.required) {
          this.required_error = true;
          return '';
        }
        if (!this.$v.form_data.cargo.maxLength) {
          this.required_error = false;
          return 'El cargo solo puede tener 30 carácteres';
        }
      }
      if (campo === 'sector_id') {
    ...
    async loadData(props) {
      ...
      await this.getPropuesta(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.savePropuesta(this.form_data);
        this.reset_form();
        this.closeModal();
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editPropuesta(this.form_data);
        this.reset_form();
        this.closeModal();
        this.loadData({ pagination: this.pagination, filter: this.filter });
        this.setLoading(false);
    ...
    deletePropuestas(selected) {
      if (selected) {
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deletePropuesta(selected.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
