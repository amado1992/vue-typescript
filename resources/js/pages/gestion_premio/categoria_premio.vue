        ...
        >
          <template v-slot:top>
            <div class="text-subtitle1 text-weight-bold text-uppercase">
              Gestionar categoría de premios
            </div>
            <q-space></q-space>
            <q-input
              dense
            ...
            <q-btn
              key="filter"
              dense
              :disable="categoria_premio.length != 0 ? false : true"
            ...
            <q-td :props="props">
              <div class="q-gutter-sm">
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteCategoria_premios(props.row)"
                >
            ...
            <q-toolbar-title>
              {{
                this.accion == 'adicionar'
                  ? 'Adicionar categoría de premios'
                  : 'Actualizar categoría de premios'
              }}
            </q-toolbar-title>
...
export default {
  ...
  computed: {
    ...mapState('categoria_premio', ['categoria_premio']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('usuario', ['getTotalUsuariosRol']),
    ...mapActions('categoria_premio', [
      'getCategoria_premio',
      'saveCategoria_premio',
      'editCategoria_premio',
      'deleteCategoria_premio',
    ]),
    ...
    async loadData(props) {
      ...
      await this.getCategoria_premio(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveCategoria_premio(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editCategoria_premio(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteCategoria_premios(selected) {
      if (selected) {
        this.setFormData(selected);
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteCategoria_premio(this.form_data.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
