  ...
  <div class="q-pa-md">
    <q-card>
      ...
      <q-card-section>
        <q-table
          class="table-title"
          title="Gestionar estado de grupo electrógeno"
                ...
                <q-toolbar-title>
                  {{
                    this.accion == "adicionar"
                      ? "Adicionar estado del grupo electrógeno"
                      : "Actualizar estado del grupo electrógeno"
                  }}
                </q-toolbar-title>
                <q-btn
                  round
            ...
            <q-card-section>
              <div class="row">
                ...
                <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 q-px-sm">
                  <q-input
                    ref="name"
                    v-model="form_create.abreviatura"
                    label="Abreviatura *"
                    lazy-rules
                    :rules="[
                      (val) =>
...
export default {
  ...
  mounted() {
    this.addToHomeBreadcrumbs([
      ...
      { label: "Nomencladores" },
      { label: "Estado de grupo electrógeno" }
    ]);
  },
  watch: {
    someText() {
  ...
  methods: {
    ...
    confirmDelete(dataRow) {
      this.$q
        .dialog({
          title: "Confirme",
          message: "¿Estás seguro que desea eliminar este estado del grupo electrógeno?",
          cancel: true,
          persistent: true,
        })
        .onOk(() => {
    ...
    async save() {
      ...
      return await axios
        .post("/api/estado_ge", this.form_create)
        .then((result) => {
          this.loadData();
    ...
    async update() {
      ...
      return await axios
        .put("/api/estado_ge/" + id, this.form_create)
        .then((result) => {
          this.loadData();
