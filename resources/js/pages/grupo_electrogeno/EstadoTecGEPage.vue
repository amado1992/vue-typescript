  ...
  <div class="q-pa-md">
    <q-card>
      ...
      <q-card-section>
        <q-table
          class="table-title"
          title="Gestionar estado técnico del grupo electrógeno"
                ...
                <q-toolbar-title>
                  {{
                    this.accion == "adicionar"
                      ? "Adicionar estado de técnico del grupo electrógeno"
                      : "Actualizar estado de técnico del grupo electrógeno"
                  }}
                </q-toolbar-title>
                <q-btn
                  round
...
export default {
  ...
  mounted() {
    this.addToHomeBreadcrumbs([
      ...
      { label: "Nomencladores" },
      { label: "Estado técnico de grupo electrógeno" }
    ]);
  },
  watch: {
    someText() {
  ...
  methods: {
    ...
    async deleteRowRequest(dataRow) {
      ...
      this.$q.loading.show();
      return await axios
        .delete("/api/estadotecnico_ge/" + id)
        .then((response) => {
          this.reset_form();
          this.$q.loading.hide();
          this.loadData();
    ...
    confirmDelete(dataRow) {
      this.$q
        .dialog({
          title: "Confirme",
          message: "¿Estás seguro que desea eliminar este estado de técnico del grupo electrógeno?",
          cancel: true,
          persistent: true,
        })
        .onOk(() => {
    ...
    async save() {
      ...
      return await axios
        .post("/api/estadotecnico_ge", this.form_create)
        .then((result) => {
          this.loadData();
    ...
    async update() {
      ...
      return await axios
        .put("/api/estadotecnico_ge/" + id, this.form_create)
        .then((result) => {
          this.loadData();
