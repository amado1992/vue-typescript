      ...
      <q-card-section>
        <div class="row">
          <div class="text-subtitle1 text-weight-bold text-uppercase">
            gestionar categorías docentes y científicas
          </div>
          <q-space></q-space>
          <q-input
            dense
                ...
                <q-toolbar-title>
                  {{
                    this.accion == "adicionar"
                      ? "Adicionar categoria docente o científica"
                      : "Actualizar categoria docente o científica"
                  }}
                </q-toolbar-title>
                <q-btn
                  round
...
export default {
  name: "CategoriaDocenteCientificaPage",
  data() {
    return {
  ...
  mounted() {
    this.addToHomeBreadcrumbs([
      ...
      { label: "Nomencladores" },
      { label: "Categorías docentes y científicas" },
    ]);
  },
  watch: {
  },
  computed: {
    isComplete() {
      if (this.form_create.nombre /*&& this.form_create.descripcion*/) {
        return false;
      }
      return true;
    },
  ...
  methods: {
    ...
    async deleteRowRequest(dataRow) {
      ...
      this.$q.loading.show();
      return await axios
        .delete("/api/categoria_docente_cientifica/" + id)
        .then((response) => {
          this.reset_form();
          this.$q.loading.hide();
          this.loadData();
    ...
    async save() {
      ...
      return await axios
        .post("/api/categoria_docente_cientifica", this.form_create)
        .then((result) => {
          this.loadData();
    ...
    async update() {
      ...
      return await axios
        .put("/api/categoria_docente_cientifica/" + id, this.form_create)
        .then((result) => {
          this.loadData();
