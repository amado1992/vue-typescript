...
export default {
  ...
  mounted() {
    this.addToHomeBreadcrumbs([
      ...
      { label: "Nomencladores" },
      { label: "Empresas asociadas a Osdes" }
    ]);
  },
  watch: {},
  computed: {
  ...
  methods: {
    ...
    confirmDelete(dataRow) {
      this.$q
        .dialog({
          title: "Confirme",
          message: "¿Estás seguro que desea eliminar este empresa?",
          cancel: true,
          persistent: true,
        })
        .onOk(() => {
    ...
    async save() {
      ...
      return await axios
        .post("/api/empresa", this.form_create)
        .then((result) => {
          this.loadData();
    ...
    async update() {
      ...
      return await axios
        .put("/api/empresa/" + id, this.form_create)
        .then((result) => {
          this.loadData();
