      ...
      <q-card-section>
        <div class="row">
          <div class="text-subtitle1 text-weight-bold text-uppercase">
            gestionar tipo de instalación TMHS
          </div>
          <q-space></q-space>
          <q-input
            dense
          ...
          <q-card-section class="no-padding">
                ...
                <q-toolbar-title>
                  {{
                    this.accion == "adicionar"
                      ? "Adicionar tipo de instalación"
                      : "Actualizar tipo de instalación"
                  }}
                </q-toolbar-title>
                <q-btn
                  round
            ...
            <q-card-section>
              <div class="row q-gutter-y-xs">
                ...
                <div class="col-xs-6 col-sm-6 col-md-6 col-lg-6 q-px-sm">
                  <q-select
                    v-model="form_create.mttipoinst_id"
                ...
                <div class="col-xs-6 col-sm-6 col-md-6 col-lg-6 q-px-sm">
                  <q-select
                    v-model="form_create.mtvalor_id"
                    :options="valores"
                    label="Valor *"
                    options-dense
                    option-value="id"
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          field: (row) => row.tipo_instalacion.nombre,
          sortable: true,
          headerClasses: "text-uppercase",
        },
          {
  ...
  created() {
    this.loadData();
    this.loadDataValores();
    this.loadDataTiposInstalaciones();
  },
  mounted() {
  ...
  computed: {
    isComplete() {
      if (
        ...
        this.form_create.mttipoinst_id &&
        this.form_create.mtvalor_id
      ) {
        return false;
      }
      return true;
  ...
  methods: {
    ...
    reset_form() {
      ...
      this.form_create.mttipoinst_id = null;
      this.form_create.mtvalor_id = null
    },
    closeModal() {
    ...
    async deleteRowRequest(dataRow) {
      ...
      this.$q.loading.show();
      return await axios
        .delete("/api/tipo_instalacion/" + id)
        .then((response) => {
          this.reset_form();
          this.$q.loading.hide();
          this.loadData();
    ...
    confirmDelete(dataRow) {
      this.$q
        .dialog({
          title: "Confirme",
          message: "¿Estás seguro que desea eliminar este tipo de instalación?",
          cancel: true,
          persistent: true,
        })
        .onOk(() => {
    ...
    async save() {
      ...
      return await axios
        .post("/api/tipo_instalacion", this.form_create)
        .then((result) => {
          this.loadData();
    ...
    async update() {
      ...
      return await axios
        .put("/api/tipo_instalacion/" + id, this.form_create)
        .then((result) => {
          this.loadData();
    ...
    async loadDataValores() {
      return await axios
        .get("/api/valores")
        .then((result) => {
          this.valores = result.data.data;
        })
        .catch((err) => {
          this.$q.notify({
            color: "negative",
    ...
    async loadDataTiposInstalaciones() {
      return await axios
        .get("/api/lists_tipos_instalaciones")
        .then((result) => {
          this.tiposInstalaciones = result.data.data;
        })
        .catch((err) => {
          this.$q.notify({
            color: "negative",
