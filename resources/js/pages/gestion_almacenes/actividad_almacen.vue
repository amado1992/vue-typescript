        ...
        >
          <template v-slot:top>
            <div class="text-subtitle1 text-weight-bold text-uppercase">
              Gestionar actividad de almacen
            </div>
            <q-space></q-space>
            <q-input
              dense
            ...
            <q-btn
              key="filter"
              dense
              :disable="actividad_almacen.length != 0 ? false : true"
            ...
            <q-td :props="props">
              <div class="q-gutter-sm">
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteActividad_almacenes(props.row)"
                >
            ...
            <q-toolbar-title>
              {{
                this.accion == 'adicionar'
                  ? 'Adicionar actividad de almacen'
                  : 'Actualizar actividad de almacen'
              }}
            </q-toolbar-title>
...
export default {
  ...
  computed: {
    ...mapState('actividad_almacen', ['actividad_almacen']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('usuario', ['getTotalUsuariosRol']),
    ...mapActions('actividad_almacen', [
      'getActividad_almacen',
      'saveActividad_almacen',
      'editActividad_almacen',
      'deleteActividad_almacen',
    ]),
    ...
    async loadData(props) {
      ...
      await this.getActividad_almacen(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveActividad_almacen(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editActividad_almacen(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteActividad_almacenes(selected) {
      if (selected) {
        this.setFormData(selected);
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteActividad_almacen(this.form_data.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
