        ...
        >
          <template v-slot:top>
            ...
            <q-btn
              key="filter"
              dense
              :disable="tipo_almacen.length != 0 ? false : true"
            ...
            <q-td :props="props">
              <div class="q-gutter-sm">
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteTipo_almacenes(props.row)"
                >
...
export default {
  ...
  computed: {
    ...mapState('tipo_almacen', ['tipo_almacen']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('usuario', ['getTotalUsuariosRol']),
    ...mapActions('tipo_almacen', [
      'getTipo_almacen',
      'saveTipo_almacen',
      'editTipo_almacen',
      'deleteTipo_almacen',
    ...
    valnombre() {
      var mio = false;
      var mio1 = false;
      for (var i = 0; i < this.tipo_almacen.length; i++) {
        if (this.form_data.nombre === this.tipo_almacen[i].nombre) {
          mio = true;
        } else {
          mio1 = true;
        }
    ...
    async loadData(props) {
      ...
      await this.getTipo_almacen(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveTipo_almacen(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editTipo_almacen(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteTipo_almacenes(selected) {
      if (selected) {
        this.setFormData(selected);
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteTipo_almacen(this.form_data.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
