        ...
        >
          <template v-slot:top>
            <div class="text-subtitle1 text-weight-bold text-uppercase">
              Gestionar temperatura de almacen
            </div>
            <q-space></q-space>
            <q-input
              dense
            ...
            <q-btn
              key="filter"
              dense
              :disable="temperatura.length != 0 ? false : true"
            ...
            <q-td :props="props">
              <div class="q-gutter-sm">
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteTemperaturas(props.row)"
                >
            ...
            <q-toolbar-title>
              {{
                this.accion == 'adicionar'
                  ? 'Adicionar temperatura de almacen'
                  : 'Actualizar temperatura de almacen'
              }}
            </q-toolbar-title>
...
export default {
  ...
  mounted() {
    this.addToHomeBreadcrumbs([
      ...
      { label: 'Almacenes' },
      { label: 'Nomencladores' },
      { label: 'Temperatura de almacenes' },
    ]);
  },
  watch: {
    pagination: {
  ...
  computed: {
    ...mapState('temperatura', ['temperatura']),
  },
  methods: {
    ...
    async loadData(props) {
      ...
      await this.getTemperatura(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveTemperatura(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editTemperatura(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteTemperaturas(selected) {
      if (selected) {
        this.setFormData(selected);
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteTemperatura(this.form_data.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
