        ...
        >
          <template v-slot:top>
            <div class="text-subtitle1 text-weight-bold text-uppercase">
              Gestionar distribución de almacen
            </div>
            <q-space></q-space>
            <q-input
              dense
            ...
            <q-btn
              key="filter"
              dense
              :disable="distribucion.length != 0 ? false : true"
            ...
            <q-td :props="props">
              <div class="q-gutter-sm">
                ...
                <q-btn
                  ...
                  text-color="negative"
                  icon="delete"
                  @click.prevent="deleteDistribuciones(props.row)"
                >
            ...
            <q-toolbar-title>
              {{
                this.accion == 'adicionar'
                  ? 'Adicionar distribución de almacen'
                  : 'Actualizar distribución de almacen'
              }}
            </q-toolbar-title>
...
export default {
  ...
  mounted() {
    this.addToHomeBreadcrumbs([
      ...
      { label: 'Almacenes' },
      { label: 'Nomencladores' },
      { label: 'Distribución de almacenes' },
    ]);
  },
  watch: {
    pagination: {
  ...
  computed: {
    ...mapState('distribucion', ['distribucion']),
  },
  methods: {
    ...mapGetters('utils', ['getterLoading']),
    ...mapActions('usuario', ['getTotalUsuariosRol']),
    ...mapActions('distribucion', [
      'getDistribucion',
      'saveDistribucion',
      'editDistribucion',
      'deleteDistribucion',
    ...
    async loadData(props) {
      ...
      await this.getDistribucion(data).then((r) => {
    ...
    async save() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.saveDistribucion(this.form_data).then(result => {
          if (result.error == 500) {
    ...
    async update() {
      ...
      if (this.$v.form_data.$invalid) {
        ...
        this.setLoading(true);
        await this.editDistribucion(this.form_data).then(result => {
          if (result.error == 500) {
            warningMessage(result.message);
          }else{
            this.reset_form();
    ...
    deleteDistribuciones(selected) {
      if (selected) {
        this.setFormData(selected);
        showDialog(message.lAskForRemove).onOk(async () => {
          this.setLoading(true);
          await this.deleteDistribucion(this.form_data.id);
          this.reset_form();
          await this.loadData({
            pagination: this.pagination,
            filter: this.filter,
