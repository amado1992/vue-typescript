<template>
  <div class="q-pa-md">
    <q-card>
      <q-card-section>
        <div class="row">
          <div class="col-6 text-h6 text-uppercase">Licencias y Avales</div>
          <q-space></q-space>
        </div>
      </q-card-section>
...
export default {
  ...
  methods: {
    ...
    ...mapActions('indicadoresl210', ['getReporteLicAvales']),
    ...
    async loadData() {
      if (this.form_data.mes === null && this.form_data.anno === null) {
        ...
        await this.getReporteLicAvales(data).then((response) => {
          this.data = response;
