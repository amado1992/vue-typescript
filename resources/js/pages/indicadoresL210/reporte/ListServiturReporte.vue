<template>
  <div class="q-pa-md">
    <q-card>
      <q-card-section>
        <div class="row">
          <div class="col-6 text-h6 text-uppercase">Solo Servitur</div>
          <q-space></q-space>
        </div>
      </q-card-section>
...
export default {
  ...
  methods: {
    ...
    ...mapActions('indicadoresl210', ['getReporteListaServitur']),
    ...
    async loadData() {
      if (this.form_data.mes === null && this.form_data.anno === null) {
        ...
        await this.getReporteListaServitur(data).then((response) => {
          this.data = response;
